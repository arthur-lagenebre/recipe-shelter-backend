...
export class AdminCommentRepositoryMysql implements AdminCommentRepository {
    ...
    async countModeratedForAdmin(): Promise<number> {
        const [rows] = await this.db.execute(`SELECT COUNT(*) AS Count FROM Comments WHERE ModeratedAt IS NOT NULL`);
    ...
    async findSoftDeletedForAdmin(): Promise<AdminComment[]> {
        const [rows] = await this.db.execute(
            `SELECT c.Id, c.RecipeId, r.Title AS RecipeTitle, r.Slug AS RecipeSlug, c.UserId, u.Username, c.ParentCommentId, c.ModeratedAt, c.ModeratedByUserId, moderator.Username AS ModeratedByUsername, c.ModerationReason, c.DeletedAt, c.DeletedByUserId, deletedBy.Username AS DeletedByUsername, c.Rating, c.Comment, c.CreatedAt, c.UpdatedAt FROM Comments AS c INNER JOIN Recipes AS r ON c.RecipeId = r.Id INNER JOIN Users AS u ON c.UserId = u.Id LEFT JOIN Users AS moderator ON c.ModeratedByUserId = moderator.Id LEFT JOIN Users AS deletedBy ON c.DeletedByUserId = deletedBy.Id WHERE c.DeletedAt IS NOT NULL ORDER BY c.DeletedAt DESC, c.Id DESC`
        );
    ...
    async countSoftDeletedForAdmin(): Promise<number> {
        const [rows] = await this.db.execute(`SELECT COUNT(*) AS Count FROM Comments WHERE DeletedAt IS NOT NULL`);
    ...
    async createModerationLog(auditLogId: number, commentId: number, db: PoolConnection): Promise<void> {
        const [result] = await db.execute<ResultSetHeader>(
            `INSERT INTO CommentModerationLogs (AdminAuditLogId, CommentId) SELECT audit.Id, ? FROM AdminAuditLogs AS audit WHERE audit.Id = ? AND audit.Action = 'comments.hide' AND audit.TargetType = 'comment' AND audit.Reason IS NOT NULL AND BINARY audit.TargetId = BINARY CAST(? AS CHAR)`,
    ...
    async unmoderate(id: number, db?: PoolConnection): Promise<boolean> {
        const [result] = await (db ?? this.db).execute<ResultSetHeader>(
            `UPDATE Comments SET ModeratedAt = NULL, ModeratedByUserId = NULL, ModerationReason = NULL WHERE Id = ? AND ModeratedAt IS NOT NULL`,
            [id]
        );
    ...
    async restore(id: number, db?: PoolConnection): Promise<boolean> {
        const [result] = await (db ?? this.db).execute<ResultSetHeader>(
            `UPDATE Comments SET DeletedAt = NULL, DeletedByUserId = NULL WHERE Id = ? AND DeletedAt IS NOT NULL`,
            [id]
        );
    ...
    async update(input: AdminUpdateCommentInput, db?: PoolConnection): Promise<AdminComment | null> {
        const [result] = await (db ?? this.db).execute<ResultSetHeader>(`UPDATE Comments SET Comment = ?, Rating = ? WHERE Id = ?`, [
            input.comment,
            input.rating ?? null,
            input.id
        ]);
    ...
    async delete(id: number, db?: PoolConnection): Promise<boolean> {
        const [result] = await (db ?? this.db).execute<ResultSetHeader>(`DELETE FROM Comments WHERE Id = ?`, [id]);
