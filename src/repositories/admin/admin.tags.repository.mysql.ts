...
export class AdminTagRepositoryMysql implements AdminTagRepository {
    ...
    async find(filters: AdminTagListFilters, pagination: PaginationOptions, db?: PoolConnection): Promise<PaginatedResult<Tag>> {
       ...
       ${limitOffsetClause}`,
        ...
        return createPaginatedResult(rows.map(mapTag), countRow ? Number(countRow.Count) : 0, pagination);
    }
    ...
    async merge(sourceTagId: number, targetTagId: number, db: PoolConnection): Promise<AdminTagMergeResult> {
        ...
        const sourceRecipeIds = new Set(
            associationRows.filter((row) => Number(row.TagId) === sourceTagId).map((row) => Number(row.RecipeId))
        );
        const targetRecipeIds = new Set(
            associationRows.filter((row) => Number(row.TagId) === targetTagId).map((row) => Number(row.RecipeId))
        );
        const deduplicatedRecipeCount = [...sourceRecipeIds].filter((recipeId) => targetRecipeIds.has(recipeId)).length;
