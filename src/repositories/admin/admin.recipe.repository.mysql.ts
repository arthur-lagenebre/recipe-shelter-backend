...
export class AdminRecipeRepositoryMysql implements AdminRecipeRepository {
    ...
    async findPendingForAdmin(): Promise<RecipePending[]> {
        const [rows] = await this.db.execute(
            `SELECT  r.Id, u.Username AS User, rc.Name AS Category, r.Title, r.Slug, r.Description, SubmittedAt FROM Recipes AS r LEFT JOIN Users AS u ON r.UserId = u.Id LEFT JOIN RecipeCategories AS rc ON r.CategoryId = rc.Id WHERE r.Status = 'pending'`
        );
    ...
    async countPendingForAdmin(): Promise<number> {
        const [rows] = await this.db.execute(`SELECT COUNT(*) AS Count FROM Recipes WHERE Status = 'pending'`);
    ...
    async findByIdForAdmin(id: number): Promise<RecipeAdmin | null> {
        const [rows] = await this.db.execute(
            `SELECT r.Id, r.UserId, u.Username, r.CategoryId, rc.Name AS Category, r.Title, r.Slug, r.Description, r.PrepTimeMinutes, r.RestTimeMinutes, r.CookTimeMinutes, r.Servings, r.Status, r.CreatedAt, r.SubmittedAt, r.ModeratedAt, r.ModeratedByUserId, r.PublishedAt, r.ArchivedAt, r.RejectionReason, r.ArchiveReason, r.UpdatedAt, ri.Id AS CoverImageId, ri.LargeStorageKey AS CoverImageLargeStorageKey, ri.MediumStorageKey AS CoverImageMediumStorageKey, ri.ThumbnailStorageKey AS CoverImageThumbnailStorageKey, ri.LargeWidth AS CoverImageWidth, ri.LargeHeight AS CoverImageHeight, ri.AltText AS CoverImageAltText FROM Recipes AS r INNER JOIN Users AS u ON r.UserId = u.Id LEFT JOIN RecipeCategories AS rc ON r.CategoryId = rc.Id LEFT JOIN RecipeImages AS ri ON ri.RecipeId = r.Id WHERE r.Id = ?`,
            [id]
        );
    ...
    async findAuditStateById(id: number, db: PoolConnection): Promise<AdminRecipeAuditState | null> {
        const [rows] = await db.execute(
            `SELECT Id, UserId, CategoryId, Title, Slug, Status, ModeratedByUserId, RejectionReason, ArchiveReason FROM Recipes WHERE Id = ? FOR UPDATE`,
            [id]
        );
    ...
    async publish(id: number, moderatedByUserId: number, db?: PoolConnection): Promise<boolean> {
        const [result] = await (db ?? this.db).execute<ResultSetHeader>(
            `UPDATE Recipes SET Status = ?, PublishedAt = CURRENT_TIMESTAMP, ModeratedByUserId = ? WHERE Id = ?`,
    ...
    async reject(id: number, moderatedByUserId: number, rejectionReason: string, db?: PoolConnection): Promise<boolean> {
        const [result] = await (db ?? this.db).execute<ResultSetHeader>(
            `UPDATE Recipes SET Status = ?, ModeratedAt = CURRENT_TIMESTAMP, ModeratedByUserId = ?, RejectionReason = ? WHERE Id = ?`,
    ...
    async archive(id: number, moderatedByUserId: number, archiveReason: string, db?: PoolConnection): Promise<boolean> {
        const [result] = await (db ?? this.db).execute<ResultSetHeader>(
            `UPDATE Recipes SET Status = 'archived', ArchivedAt = CURRENT_TIMESTAMP, ArchiveReason = ? WHERE Id = ?`,
            [archiveReason, id]
        );
    ...
    async createModerationLog(auditLogId: number, recipeId: number, db: PoolConnection): Promise<void> {
        const [result] = await db.execute<ResultSetHeader>(
            `INSERT INTO RecipeModerationLogs (AdminAuditLogId, RecipeId) SELECT audit.Id, ? FROM AdminAuditLogs AS audit WHERE audit.Id = ? AND audit.Action IN ('recipes.reject', 'recipes.archive') AND audit.TargetType = 'recipe' AND audit.Reason IS NOT NULL AND BINARY audit.TargetId = BINARY CAST(? AS CHAR)`,
    ...
    async delete(id: number, db?: PoolConnection): Promise<boolean> {
        const [result] = await (db ?? this.db).execute<ResultSetHeader>(`DELETE FROM Recipes WHERE Id = ?`, [id]);
    ...
    private async findIngredientsByRecipeId(recipeId: number): Promise<RecipeIngredientRow[]> {
        const [rows] = await this.db.execute(
            `SELECT ri.IngredientId, i.Name, ri.DisplayText, ri.Quantity, ri.Unit, ri.Note, ri.SortOrder FROM RecipeIngredients AS ri LEFT JOIN Ingredients AS i ON ri.IngredientId = i.Id WHERE RecipeId = ? ORDER BY ri.SortOrder, ri.Id`,
            [recipeId]
        );
    ...
    private async findEquipmentsByRecipeId(recipeId: number): Promise<RecipeEquipmentRow[]> {
        const [rows] = await this.db.execute(
            `SELECT e.Id, e.Name FROM RecipeEquipments AS re INNER JOIN Equipments AS e ON re.EquipmentId = e.Id WHERE RecipeId = ?`,
            [recipeId]
        );
    ...
    private async findTagsByRecipeId(recipeId: number): Promise<RecipeTagRow[]> {
        const [rows] = await this.db.execute(
            `SELECT t.Id, t.Name FROM RecipeTags AS rt INNER JOIN Tags AS t ON rt.TagId = t.Id WHERE rt.RecipeId = ? ORDER BY t.Name ASC`,
            [recipeId]
        );
