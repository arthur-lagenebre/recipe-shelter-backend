...
import type {
    ...
    AdminIngredientAliasWriteResult,
    ...
    AdminIngredientWriteResult
} from './admin.ingredients.types.js';
...
export class AdminIngredientRepositoryMysql implements AdminIngredientRepository {
    ...
    async hasAliases(ingredientId: number, db: PoolConnection): Promise<boolean> {
        const [rows] = await db.execute<RowDataPacket[]>(`SELECT 1 FROM IngredientAliases WHERE IngredientId = ? LIMIT 1`, [ingredientId]);

        return rows.length > 0;
    }

    async hasMergedSources(ingredientId: number, db: PoolConnection): Promise<boolean> {
        const [rows] = await db.execute<RowDataPacket[]>(`SELECT 1 FROM Ingredients WHERE MergedIntoIngredientId = ? LIMIT 1`, [
            ingredientId
        ]);
    ...
    async merge(input: AdminIngredientMergeInput, db: PoolConnection): Promise<AdminIngredientMergeResult> {
        ...
        const sourceNameAlias = aliasRows.find(
            (row) => row.LanguageCode === sourceNameLanguageCode && row.NormalizedName === sourceNormalizedName
        );
        const sourceNameAliasIngredientId = sourceNameAlias ? Number(sourceNameAlias.IngredientId) : null;
        ...
        const sourceNameAliasResolution =
            sourceNameAliasIngredientId === sourceIngredientId
        ...
        if (sourceNameAliasResolution === 'created') {
            try {
                ...
                if (isDuplicateAlias(error)) {
                    return {
                        status: 'source_name_alias_conflict',
                        conflictingIngredientId: await findAliasIngredientId(db, sourceNameLanguageCode, sourceNormalizedName)
                    };
                }
        ...
        return {
            ...
            targetAliasCountAfter: targetAliasCount + aliasResult.affectedRows + (sourceNameAliasResolution === 'created' ? 1 : 0),
...
function isDuplicateAlias(error: unknown): boolean {
    return (
        isDuplicateEntry(error) && 'message' in error && String(error.message).includes('ingredient_aliases_language_normalized_name_UK')
    );
}
