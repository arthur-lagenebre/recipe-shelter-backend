...
export class CommentRepositoryMysql implements CommentRepository {
    ...
    async create(input: CreateCommentInput): Promise<PublicComment> {
        const [result] = await this.db.execute(
            `INSERT INTO Comments (RecipeId, UserId, ParentCommentId, ModeratedAt, ModeratedByUserId, Rating, Comment) VALUES (?, ?, ?, ?, ?, ?, ?);`,
    ...
    async update(input: UpdateCommentInput): Promise<PublicComment | null> {
        const [result] = await this.db.execute<ResultSetHeader>(
            `UPDATE Comments SET Comment = ?, Rating = ? WHERE Id = ? AND UserId = ? AND DeletedAt IS NULL`,
    ...
    async softDelete(id: number, userId: number): Promise<boolean> {
        const [result] = await this.db.execute<ResultSetHeader>(
            `UPDATE Comments SET DeletedAt = CURRENT_TIMESTAMP, DeletedByUserId = ? WHERE Id = ? AND UserId = ? AND DeletedAt IS NULL`,
            [userId, id, userId]
        );
    ...
    async findById(id: number): Promise<Comment | null> {
        const [rows] = await this.db.execute(
            `SELECT Id, RecipeId, UserId, ParentCommentId, ModeratedAt, ModeratedByUserId, DeletedAt, DeletedByUserId, Rating, Comment, CreatedAt, UpdatedAt FROM Comments WHERE Id = ? AND DeletedAt IS NULL AND ModeratedAt IS NULL`,
            [id]
        );
    ...
    private async findPublicById(id: number): Promise<PublicComment | null> {
        const [rows] = await this.db.execute(
            `SELECT c.Id, c.RecipeId, u.Id AS AuthorId, u.Username AS AuthorUsername, c.ParentCommentId, c.ModeratedAt, c.DeletedAt, c.Rating, c.Comment, c.CreatedAt, c.UpdatedAt FROM Comments AS c INNER JOIN Users AS u ON u.Id = c.UserId WHERE c.Id = ? AND c.DeletedAt IS NULL AND c.ModeratedAt IS NULL`,
            [id]
        );
