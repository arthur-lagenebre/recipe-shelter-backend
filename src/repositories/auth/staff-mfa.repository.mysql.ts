...
} from './staff-mfa.repository.interface.js';
...
export class StaffMfaRepositoryMysql implements StaffMfaRepository {
    ...
    async saveChallenge(input: CreateStaffWebAuthnChallengeInput): Promise<boolean> {
...
function mapCredential(row: CredentialRow): StaffWebAuthnCredential {
    const transports =
        typeof row.Transports === 'string' ? (JSON.parse(row.Transports) as AuthenticatorTransportFuture[]) : (row.Transports ?? []);
