...
export class CategoryRepositoryMysql implements CategoryRepository {
    ...
    async findAll(): Promise<Category[]> {
        const [rows] = await this.db.execute(`SELECT Id, Name, Slug, IconName, CreatedAt, UpdatedAt FROM RecipeCategories`);

        return (rows as CategoryRow[]).map(mapCategory);
    }

    async findById(id: number): Promise<Category | null> {
        const [rows] = await this.db.execute(`SELECT Id, Name, Slug, IconName, CreatedAt, UpdatedAt FROM RecipeCategories WHERE Id = ?`, [
            id
        ]);
