...
export class UserService {
    ...
    async getUser(username: string, viewerUserId: number | null = null): Promise<PublicUserWithPublishedRecipes> {
        ...
        if (user.accountType !== 'community') throw notFound('User not found', 'USER_NOT_FOUND');
    ...
    async updateEmail(userId: number, newEmail: string, currentPassword: string): Promise<PublicUserProfile> {
        ...
        if (!normalizedEmail) throw badRequest('New email is required', 'USERS_UPDATE_EMAIL_MISSING_EMAIL');

        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

        if (!emailRegex.test(normalizedEmail)) throw badRequest('Invalid email format', 'USERS_UPDATE_EMAIL_INVALID_EMAIL');

        const user = await this.userRepository.findWithPasswordById(userId);

        if (!user) throw notFound('User not found', 'USER_NOT_FOUND');

        if (!user.passwordHash) throw unauthorized('Current password is incorrect', 'USERS_UPDATE_EMAIL_BAD_PASSWORD');

        const passwordOk = await bcrypt.compare(currentPassword, user.passwordHash);

        if (!passwordOk) throw unauthorized('Current password is incorrect', 'USERS_UPDATE_EMAIL_BAD_PASSWORD');
        ...
        if (existingUser && existingUser.id !== userId) throw conflict('Email already in use', 'USERS_UPDATE_EMAIL_ALREADY_USED');
    ...
    async updatePassword(userId: number, currentPassword: string, newPassword: string, currentSessionId: string | null): Promise<void> {
        ...
        if (!user.passwordHash) throw unauthorized('Current password is incorrect', 'USERS_UPDATE_PASSWORD_BAD_CURRENT');

        const passwordOk = await bcrypt.compare(currentPassword, user.passwordHash);

        if (!passwordOk) throw unauthorized('Current password is incorrect', 'USERS_UPDATE_PASSWORD_BAD_CURRENT');
        ...
        if (passwordError) throw badRequest(passwordError, 'USERS_UPDATE_PASSWORD_WEAK_PASSWORD');
    ...
    async updateUsername(userId: number, currentPassword: string, newUsername: string): Promise<PublicUserProfile> {
        ...
        if (!username) throw badRequest('New username is required', 'USERS_UPDATE_USERNAME_MISSING_USERNAME');

        if (username.length < 3) throw badRequest('Username too short', 'USERS_UPDATE_USERNAME_WEAK_USERNAME');

        const user = await this.userRepository.findWithPasswordById(userId);

        if (!user) throw notFound('User not found', 'USER_NOT_FOUND');

        if (!user.passwordHash) throw unauthorized('Current password is incorrect', 'USERS_UPDATE_USERNAME_BAD_PASSWORD');

        const passwordOk = await bcrypt.compare(currentPassword, user.passwordHash);

        if (!passwordOk) throw unauthorized('Current password is incorrect', 'USERS_UPDATE_USERNAME_BAD_PASSWORD');
        ...
        if (existingUser && existingUser.id !== userId) throw conflict('Username already in use', 'USERS_UPDATE_USERNAME_ALREADY_USED');
