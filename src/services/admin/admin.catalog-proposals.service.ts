...
import type { CatalogProposal, CatalogProposalListFilters, CatalogProposalType } from '../../repositories/catalog/catalog-proposals.types.js';
...
export class AdminCatalogProposalService {
  constructor(private readonly proposals: AdminCatalogProposalRepository, private readonly tags: AdminTagRepository, private readonly ingredients: AdminIngredientRepository, private readonly auditActions: AdminAuditActionRunner) { }

  async list(filters: CatalogProposalListFilters, pagination: PaginationOptions, actorUserId: number, context: AdminAuditRequestContext): Promise<PaginatedResult<CatalogProposal>> {
  ...
  async acceptTag(proposalId: number, input: AcceptTagCatalogProposalCommand, actorUserId: number, context: AdminAuditRequestContext): Promise<CatalogProposal> {
    ...
    return this.auditActions.run(async ({ db, audit }) => {
      ...
      const tag = requireWrittenTag(await this.tags.create({
  ...
  async acceptIngredient(proposalId: number, input: AcceptIngredientCatalogProposalCommand, actorUserId: number, context: AdminAuditRequestContext): Promise<CatalogProposal> {
    requirePositiveId(proposalId, 'Catalog proposal id', 'ADMIN_CATALOG_PROPOSALS_BAD_ID');
    ...
    return this.auditActions.run(async ({ db, audit }) => {
      ...
      await this.requireUnusedCatalogName(before, db);
      const ingredient = requireWrittenIngredient(await this.ingredients.create({
        name: before.proposedName,
        normalizedName: before.normalizedName,
        slug: command.slug ?? before.normalizedName.replace(/ /g, '-')
      }, db));
  ...
  async associateTag(proposalId: number, input: AssociateTagCatalogProposalCommand, actorUserId: number, context: AdminAuditRequestContext): Promise<CatalogProposal> {
    requirePositiveId(proposalId, 'Catalog proposal id', 'ADMIN_CATALOG_PROPOSALS_BAD_ID');
  ...
  async associateIngredient(proposalId: number, input: AssociateIngredientCatalogProposalCommand, actorUserId: number, context: AdminAuditRequestContext): Promise<CatalogProposal> {
    requirePositiveId(proposalId, 'Catalog proposal id', 'ADMIN_CATALOG_PROPOSALS_BAD_ID');
  ...
  async convertIngredientToAlias(proposalId: number, input: ConvertCatalogProposalToAliasCommand, actorUserId: number, context: AdminAuditRequestContext): Promise<CatalogProposal> {
    requirePositiveId(proposalId, 'Catalog proposal id', 'ADMIN_CATALOG_PROPOSALS_BAD_ID');
    ...
    return this.auditActions.run(async ({ db, audit }) => {
      ...
      requireActiveIngredientTarget(ingredient);
      await this.requireUnusedCatalogName(before, db);
      const alias = requireWrittenAlias(await this.ingredients.createAlias({
  ...
  private async requirePendingProposal(proposalId: number, expectedType: CatalogProposalType | undefined, db: Parameters<AdminCatalogProposalRepository['findByIdForUpdate']>[1]): Promise<CatalogProposal> {
  ...
  private async requireUnusedCatalogName(proposal: CatalogProposal, db: Parameters<AdminCatalogProposalRepository['findByIdForUpdate']>[1]): Promise<void> {
  ...
  private async reviewProposal(proposal: CatalogProposal, status: 'accepted' | 'rejected' | 'merged', matchedCatalogId: number | null, actorUserId: number, reason: string, db: Parameters<AdminCatalogProposalRepository['review']>[1]): Promise<CatalogProposal> {
...
function validateListFilters(filters: CatalogProposalListFilters): CatalogProposalListFilters {
  ...
  if (filters.q !== undefined && (typeof filters.q !== 'string' || !filters.q.trim() || filters.q.trim().length > SEARCH_MAX_LENGTH))
    throw badRequest(`Catalog proposal search must contain at most ${SEARCH_MAX_LENGTH} characters`, 'ADMIN_CATALOG_PROPOSALS_BAD_SEARCH');
...
function validateAliasCommand(input: ConvertCatalogProposalToAliasCommand) {
  ...
  return {
    targetIngredientId: requirePositiveId(input.targetIngredientId, 'Target ingredient id', 'ADMIN_CATALOG_PROPOSALS_BAD_TARGET_INGREDIENT_ID'),
    languageCode,
...
function validateOptionalSlug(value: unknown, proposalType: CatalogProposalType): string | undefined {
  ...
  if (!slug || slug.length > SLUG_MAX_LENGTH || !SLUG_PATTERN.test(slug))
    throw badRequest(`${proposalType === 'tag' ? 'Tag' : 'Ingredient'} slug is invalid`, `ADMIN_CATALOG_PROPOSALS_${proposalType.toUpperCase()}_SLUG_INVALID`);
...
function validateOptionalDescription(value: unknown): string | null {
  ...
  if (!description || description.length > DESCRIPTION_MAX_LENGTH)
    throw badRequest('Tag description must be non-blank and at most 1000 characters', 'ADMIN_CATALOG_PROPOSALS_TAG_DESCRIPTION_INVALID');
