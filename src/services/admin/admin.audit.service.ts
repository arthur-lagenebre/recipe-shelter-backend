...
} from '../../repositories/admin/admin.audit.repository.interface.js';
...
export type AdminAuditRequestContext = Pick<AdminAuditRecordInput, 'correlationId' | 'ipAddress' | 'userAgent'>;
...
export class AdminAuditService {
    constructor(
        private readonly auditLogs: AdminAuditRepository,
    ...
    async record(input: AdminAuditRecordInput): Promise<AdminAuditRecordReceipt> {
        ...
        try {
            ...
            throw internalError('Administrative action could not be audited', 'ADMIN_AUDIT_RECORD_FAILED');
        }
    }
}
...
function normalizeInput(input: AdminAuditRecordInput, correlationId: string): CreateAdminAuditLogInput {
    ...
    if (!EVENT_TYPES.has(input.eventType)) throw new TypeError('eventType must be part of the administrative audit catalog');
    if (!TARGET_TYPES.has(input.targetType)) throw new TypeError('targetType must be part of the administrative audit catalog');
    if (ADMIN_AUDIT_EVENT_TARGET_TYPES[input.eventType] !== input.targetType) throw new TypeError('targetType does not match eventType');
...
function normalizeTargetId(targetId: number | string): string {
    ...
    if (!normalized) throw new TypeError('targetId is required');
    if (normalized.length > TARGET_ID_MAX_LENGTH) throw new TypeError(`targetId must be at most ${TARGET_ID_MAX_LENGTH} characters`);
...
function normalizeOptionalText(value: string | null | undefined, maxLength: number, fieldName: string): string | null {
    ...
    if (!normalized) throw new TypeError(`${fieldName} cannot be blank`);
    if (normalized.length > maxLength) throw new TypeError(`${fieldName} must be at most ${maxLength} characters`);
...
function normalizeCorrelationId(value: string): string {
    const normalized = value.trim();
    if (!UUID_PATTERN.test(normalized)) throw new TypeError('correlationId must be a UUID');
...
function sanitizeSnapshot(snapshot: AdminAuditSnapshot | null | undefined, fieldName: string): AdminAuditSnapshot | null {
    ...
    if (!isPlainObject(snapshot)) throw new TypeError(`${fieldName} must be a plain JSON object`);

    return sanitizeObject(snapshot, new WeakSet<object>(), fieldName);
}

function sanitizeObject(
    value: Readonly<Record<string, AdminAuditJsonValue>>,
    ancestors: WeakSet<object>,
    fieldName: string
): AdminAuditSnapshot {
    if (ancestors.has(value)) throw new TypeError(`${fieldName} must not contain circular references`);

    ancestors.add(value);
    const sanitized: Record<string, AdminAuditJsonValue> = {};

    for (const [key, child] of Object.entries(value)) {
        sanitized[key] = isSensitiveFieldName(key) ? REDACTED_VALUE : sanitizeJsonValue(child, ancestors, `${fieldName}.${key}`);
    }
...
function sanitizeJsonValue(value: AdminAuditJsonValue, ancestors: WeakSet<object>, fieldName: string): AdminAuditJsonValue {
    ...
    if (typeof value === 'number') {
        if (!Number.isFinite(value)) throw new TypeError(`${fieldName} must contain only finite numbers`);
        return value;
    }
    ...
    if (isPlainObject(value)) return sanitizeObject(value, ancestors, fieldName);
...
function isSensitiveFieldName(fieldName: string): boolean {
    ...
    return SENSITIVE_FIELD_NAMES.has(normalized) || SENSITIVE_FIELD_FRAGMENTS.some((fragment) => normalized.includes(fragment));
}
