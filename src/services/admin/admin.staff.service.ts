...
export class AdminStaffService {
    constructor(
        private readonly staff: AdminStaffRepository,
    ...
    async grantRole(
        ...
        if (staffUserId === actorStaffUserId)
            throw forbidden('Staff users cannot grant roles to themselves', 'STAFF_ROLE_GRANT_SELF_FORBIDDEN');
    ...
    private async changeRole(
        ...
        return this.auditActions.run(async ({ db, audit }) => {
            const isLastActiveSuperAdmin = action === 'revoke' ? await this.staff.lockAndCheckLastActiveSuperAdmin(staffUserId, db) : false;
    ...
    private async recordLifecycleAudit(
        ...
        eventType: typeof ADMIN_AUDIT_EVENT_TYPES.staffDisable | typeof ADMIN_AUDIT_EVENT_TYPES.staffEnable,
        before: AdminStaffAccount,
        after: AdminStaffAccount,
        actorStaffUserId: number,
        reason: string,
        context: AdminAuditRequestContext,
...
function lastActiveSuperAdminConflict() {
    return conflict('The last active SuperAdmin cannot be disabled or lose the SuperAdmin role', 'LAST_ACTIVE_SUPER_ADMIN');
}
