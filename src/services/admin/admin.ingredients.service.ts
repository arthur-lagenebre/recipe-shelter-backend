...
} from '../../repositories/admin/admin.ingredients.types.js';
...
export class AdminIngredientService {
    ...
    async create(input: AdminCreateIngredientCommand, actorUserId: number, context: AdminAuditRequestContext): Promise<Ingredient> {
    ...
    async deprecate(ingredientId: number, reason: string, actorUserId: number, context: AdminAuditRequestContext): Promise<Ingredient> {
        requirePositiveId(ingredientId, 'Ingredient id', 'ADMIN_INGREDIENTS_BAD_ID');
    ...
    async restore(ingredientId: number, reason: string, actorUserId: number, context: AdminAuditRequestContext): Promise<Ingredient> {
        requirePositiveId(ingredientId, 'Ingredient id', 'ADMIN_INGREDIENTS_BAD_ID');
    ...
    async updateAlias(
        ...
        return this.auditActions.run(async ({ db, audit }) => {
            ...
            const alias = requireWrittenAlias(
                await this.ingredients.updateAlias(
                    {
                        id: aliasId,
                        ingredientId,
                        name,
    ...
    async deleteAlias(ingredientId: number, aliasId: number, actorUserId: number, context: AdminAuditRequestContext): Promise<void> {
        requirePositiveId(ingredientId, 'Ingredient id', 'ADMIN_INGREDIENTS_BAD_ID');
        requirePositiveId(aliasId, 'Ingredient alias id', 'ADMIN_INGREDIENT_ALIASES_BAD_ID');
    ...
    private async requireIngredientForUpdate(
        ingredientId: number,
...
function normalizeAndValidateName(name: string): string {
    ...
    if (normalizedName.length > INGREDIENT_NAME_MAX_LENGTH)
        throw badRequest(
            ...
            'ADMIN_INGREDIENTS_NAME_TOO_LONG'
        );
...
function normalizeAndValidateAliasName(name: string): string {
    ...
    if (normalizedName.length > INGREDIENT_NAME_MAX_LENGTH)
        throw badRequest(
            `Normalized ingredient alias name must be at most ${INGREDIENT_NAME_MAX_LENGTH} characters`,
            'ADMIN_INGREDIENT_ALIASES_NAME_TOO_LONG'
        );
