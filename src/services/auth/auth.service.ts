...
export class AuthService {
    ...
    async completeStaffLogin(input: {
        flowId: string;
    ...
    async activateStaffInvitation(input: {
        flowId: string;
    ...
    private withoutPassword(authUser: UserWithPassword): User {
        const { passwordHash: _passwordHash, staffSessionVersion: _staffSessionVersion, ...user } = authUser;
        return user;
    }
    ...
    private async createSession(
        user: User,
        realm: SessionRealm,
        mfa?: { sessionVersion: number; credentialId: string; verifiedAt: Date; ipAddress: string | null; userAgent: string | null }
    ): Promise<string> {
