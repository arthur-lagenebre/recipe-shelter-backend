...
export class RecipeImageProcessor {
    async process(input: Buffer): Promise<ProcessedRecipeImage> {
        ...
        const [large, medium, thumbnail] = await Promise.all([ this.createVariant(input, VARIANT_SIZES.large), this.createVariant(input, VARIANT_SIZES.medium), this.createVariant(input, VARIANT_SIZES.thumbnail) ]);
...
function detectFormat(input: Buffer): DetectedFormat {
    ...
    const unsupportedBinary = input.toString('ascii', 0, 6).startsWith('GIF8')
