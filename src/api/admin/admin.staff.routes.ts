...
export function createAdminStaffRouter(controller: AdminStaffController) {
    ...
    router.post(
        '/:staffUserId/disable',
        requireStaffAuth,
        RequirePermission(PERMISSIONS.staffDisable),
        requireRecentStaffAuthentication,
        controller.disable
    );
    router.post('/:staffUserId/enable', requireStaffAuth, RequirePermission(PERMISSIONS.staffEnable), controller.enable);
    router.post(
        '/:staffUserId/roles/:roleCode',
        requireStaffAuth,
        RequirePermission(PERMISSIONS.staffRoleGrant),
        requireRecentAuthenticationForSuperAdminChange,
        controller.grantRole
    );
    router.delete(
        '/:staffUserId/roles/:roleCode',
        requireStaffAuth,
        RequirePermission(PERMISSIONS.staffRoleRevoke),
        requireRecentAuthenticationForSuperAdminChange,
