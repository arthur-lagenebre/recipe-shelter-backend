...
export function parseCreateStaffInvitationBody(body: unknown): CreateStaffInvitationCommand {
    if (!isRecord(body)) throw badRequest('Invalid body', 'STAFF_INVITATION_BAD_BODY');
    ...
    if (body.roles.length > MAX_ROLE_COUNT) throw badRequest('Too many initial roles', 'STAFF_INVITATION_ROLES_INVALID');
