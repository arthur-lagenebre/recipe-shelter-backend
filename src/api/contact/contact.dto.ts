...
function getRequiredString(obj: Record<string, unknown>, field: keyof ContactMessageDto): string {
    ...
    if (typeof value !== 'string') throw badRequest('Missing fields', 'CONTACT_MISSING_FIELDS');

    const trimmedValue = value.trim();

    if (!trimmedValue) throw badRequest('Missing fields', 'CONTACT_MISSING_FIELDS');

    return trimmedValue;
}

function ensureLength(value: string, field: string, min: number, max: number): void {
    if (value.length < min) throw badRequest(`${field} is too short`, `CONTACT_${field.toUpperCase()}_TOO_SHORT`);

    if (value.length > max) throw badRequest(`${field} is too long`, `CONTACT_${field.toUpperCase()}_TOO_LONG`);
}
...
export function parseContactMessageBody(body: unknown): ContactMessageDto {
    ...
    if (!EMAIL_PATTERN.test(email)) throw badRequest('Invalid email', 'CONTACT_INVALID_EMAIL');
