...
async function createCatalogProposalTestApp() {
  ...
  configureAuthRbacRepository({ async findPermissionCodesByStaffUserId() { return []; } });
  ...
  app.use(
    '/api/v1/catalog',
    createCatalogProposalsRouter(createCatalogProposalsController(new CatalogProposalService(repository)))
  );
  app.use(notFound);
...
function postProposal(baseUrl: string, endpoint: 'tag-proposals' | 'ingredient-proposals', cookie: string | undefined, body: unknown): Promise<Response> {
...
describe('catalog proposal HTTP integration', () => {
  ...
  it('validates ownership, payloads, canonical duplicates and pending duplicates', async (context) => {
    ...
    const canonicalDuplicate = await postProposal(server.baseUrl, 'ingredient-proposals', activeCookie, { recipeId: 42, name: 'Sel fin' });
    assert.equal(canonicalDuplicate.status, 409);
    assert.equal((await canonicalDuplicate.json() as { error: { code: string } }).error.code, 'CATALOG_PROPOSALS_CANONICAL_NAME_EXISTS');

    assert.equal((await postProposal(server.baseUrl, 'tag-proposals', activeCookie, { recipeId: 42, name: 'Cuisine solaire' })).status, 201);
    const pendingDuplicate = await postProposal(server.baseUrl, 'tag-proposals', activeCookie, { recipeId: 42, name: 'CUISINE---SOLAIRE!!!' });
