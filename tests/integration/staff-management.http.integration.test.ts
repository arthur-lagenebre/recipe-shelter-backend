...
describe('staff management HTTP integration', () => {
    ...
    it('requires recent strong authentication for global revocation and SuperAdmin changes only', async () => {
        ...
        assert.equal(((await disable.json()) as { error: { code: string } }).error.code, 'AUTH_RECENT_AUTHENTICATION_REQUIRED');
        ...
        for (const roleCode of ['SuperAdmin', 'superadmin']) {
            ...
            assert.equal(((await grantSuperAdmin.json()) as { error: { code: string } }).error.code, 'AUTH_RECENT_AUTHENTICATION_REQUIRED');
        }
    ...
    it('rejects every privileged self-action with a stable business error and no side effect', async () => {
        ...
        assert.equal(((await disableSelf.json()) as { error: { code: string } }).error.code, 'STAFF_DISABLE_SELF_FORBIDDEN');
        ...
        assert.equal(((await grantSelf.json()) as { error: { code: string } }).error.code, 'STAFF_ROLE_GRANT_SELF_FORBIDDEN');
        ...
        assert.equal(((await revokeOwnRole.json()) as { error: { code: string } }).error.code, 'STAFF_ROLE_REVOKE_SELF_FORBIDDEN');

        const revokeOwnLastSuperAdminRole = await fetch(
            `${server.baseUrl}/api/v1/admin/staff/${protectedSuperAdminUser.id}/roles/SuperAdmin`,
            {
                method: 'DELETE',
    ...
    it('returns LAST_ACTIVE_SUPER_ADMIN when another actor targets the final active administrator', async () => {
        ...
        assert.equal(((await disable.json()) as { error: { code: string } }).error.code, 'LAST_ACTIVE_SUPER_ADMIN');
        ...
        assert.equal(((await revoke.json()) as { error: { code: string } }).error.code, 'LAST_ACTIVE_SUPER_ADMIN');
        assert.equal(audit.inputs.length, 0);
    });
});
