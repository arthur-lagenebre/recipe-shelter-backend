...
describe('admin catalog proposal HTTP integration', () => {
  ...
  before(async () => {
    configureAuthUserRepository({
      async findById(id) { return id === staff.id ? staff : null; }
    });
    configureAuthRbacRepository({
      async findPermissionCodesByStaffUserId() { return [...permissions]; }
    });
    ...
    adminRouter.use(
      ...
      createAdminCatalogProposalsRouter(
        createAdminCatalogProposalsController(service as unknown as AdminCatalogProposalService)
      )
    );
...
function reviewedProposal(
  status: CatalogProposal['status'],
  overrides: Partial<CatalogProposal> = {}
): CatalogProposal {
  return {
    ...pendingProposal,
    status,
