...
describe('RBAC schema and seed integration', { skip: !mysqlEnabled && 'Set TEST_DB_NAME to run isolated MySQL tests' }, () => {
    ...
    it('bootstraps the final RBAC model from an empty database', async () => {
        const databaseName = requireRbacTestDatabaseName();
        const [tables] = await connection.query(`SELECT TABLE_NAME AS TableName FROM information_schema.TABLES WHERE TABLE_SCHEMA = ? AND TABLE_NAME IN ('Roles', 'Permissions', 'StaffRoles', 'RolePermissions') ORDER BY TABLE_NAME`, [databaseName]);
        assert.deepEqual(
        ...
        const [roles] = await connection.query(`SELECT Code, Name, Description FROM Roles ORDER BY Code`);
        ...
        const [permissions] = await connection.query(`SELECT Code, Description FROM Permissions ORDER BY Code`);
        assert.deepEqual(
        ...
        const [rolePermissions] = await connection.query(`SELECT r.Code AS RoleCode, p.Code AS PermissionCode FROM RolePermissions AS rp INNER JOIN Roles AS r ON r.Id = rp.RoleId INNER JOIN Permissions AS p ON p.Id = rp.PermissionId ORDER BY r.Code, p.Code`);
    ...
    it('replays the seed without duplicating RBAC data', async () => {
        ...
        const [roleInstances] = await connection.query(`SELECT Code, COUNT(*) AS InstanceCount FROM Roles GROUP BY Code ORDER BY Code`);
        ...
        const [staffRoleAssignments] = await connection.query(`SELECT COUNT(*) AS AssignmentCount FROM StaffRoles`);
        assert.deepEqual(staffRoleAssignments, [{ AssignmentCount: 0 }]);

        const [permissionInstances] = await connection.query(`SELECT Code, COUNT(*) AS InstanceCount FROM Permissions GROUP BY Code ORDER BY Code`);
        assert.deepEqual(
            permissionInstances,
            [...Object.values(PERMISSIONS)].sort().map((Code) => ({ Code, InstanceCount: 1 }))
        );

        const [superAdminPermissions] = await connection.query(`SELECT COUNT(*) AS PermissionCount FROM RolePermissions AS rp INNER JOIN Roles AS r ON r.Id = rp.RoleId WHERE r.Code = 'SuperAdmin'`);
    ...
    it('restricts community content ownership to community profiles in a fresh schema', async () => {
        const databaseName = requireRbacTestDatabaseName();
        const [foreignKeys] = await connection.query(`SELECT TABLE_NAME AS TableName, REFERENCED_TABLE_NAME AS ReferencedTableName FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = ? AND CONSTRAINT_NAME IN ('recipes_user_FK', 'comments_user_FK', 'favorites_user_FK') ORDER BY TABLE_NAME`, [databaseName]);
        assert.deepEqual(
        ...
        await connection.query(`INSERT INTO Users (Id, Mail, Username, Password, AccountType, Status) VALUES (110, 'community-content@test.local', 'community-content', 'hash', 'community', 'active'), (111, 'staff-content@test.local', 'staff-content', 'hash', 'staff', 'inactive'); INSERT INTO RecipeCategories (Id, Name, Slug, IconName) VALUES (100, 'Community boundary', 'community-boundary', 'boundary'); INSERT INTO Recipes (Id, UserId, CategoryId, Title, Slug, Description, PrepTimeMinutes, Servings) VALUES (200, 110, 100, 'Community recipe', 'community-recipe', 'Allowed owner', 5, 2); INSERT INTO Favorites (UserId, RecipeId) VALUES (110, 200); INSERT INTO Comments (RecipeId, UserId, Comment) VALUES (200, 110, 'Allowed author')`);

        await assert.rejects(() => connection.query(`INSERT INTO Recipes (Id, UserId, CategoryId, Title, Slug, Description, PrepTimeMinutes, Servings) VALUES (201, 111, 100, 'Staff recipe', 'staff-recipe', 'Forbidden owner', 5, 2)`));
        await assert.rejects(() => connection.query(`INSERT INTO Favorites (UserId, RecipeId) VALUES (111, 200)`));
        await assert.rejects(() => connection.query(`INSERT INTO Comments (RecipeId, UserId, Comment) VALUES (200, 111, 'Forbidden author')`));
    });

    it('enforces unique assignments, foreign keys and default deny', async () => {
        await connection.query(`INSERT INTO Users (Id, Mail, Username, Password, AccountType, Status) VALUES (100, 'community-rbac@test.local', 'community-rbac', 'hash', 'community', 'active'), (101, 'staff-rbac@test.local', 'staff-rbac', 'hash', 'staff', 'inactive')`);
        ...
        await assert.rejects(() => connection.query(`INSERT INTO Roles (Code, Name, Description) VALUES ('RECIPEMODERATOR', 'Autre rôle recettes', 'Duplicate code')`));
        await assert.rejects(() => connection.query(`INSERT INTO Roles (Code, Name, Description) VALUES ('OtherRecipeModerator', 'MODÉRATEUR DE RECETTES', 'Duplicate name')`));
    ...
    it('creates strictly separate session stores and requires WebAuthn before staff activation', async () => {
        const [sessionTables] = await connection.query(`SELECT TABLE_NAME AS TableName FROM information_schema.TABLES WHERE TABLE_SCHEMA = ? AND TABLE_NAME IN ('CommunitySessions', 'StaffSessions', 'StaffWebAuthnChallenges', 'StaffWebAuthnCredentials') ORDER BY TABLE_NAME`, [requireRbacTestDatabaseName()]);
        assert.deepEqual(
        ...
        await connection.query(`INSERT INTO Users (Id, Mail, Username, Password, AccountType, Status) VALUES (120, 'session-community@test.local', 'session-community', 'hash', 'community', 'active'), (121, 'session-staff@test.local', 'session-staff', 'hash', 'staff', 'inactive')`);

        await assert.rejects(() => connection.query(`UPDATE Users SET Status = 'active' WHERE Id = 121`));
        await assert.rejects(() => connection.query(`UPDATE StaffProfiles SET MfaEnrolledAt = CURRENT_TIMESTAMP, Status = 'active' WHERE UserId = 121`));
        await connection.query(`INSERT INTO StaffWebAuthnCredentials (CredentialId, StaffUserId, PublicKey, SignatureCounter, Transports, DeviceType, BackedUp, Aaguid) VALUES ('credential-121', 121, 0x0102, 0, JSON_ARRAY('usb'), 'singleDevice', FALSE, '00000000-0000-0000-0000-000000000000'); UPDATE StaffProfiles SET MfaEnrolledAt = CURRENT_TIMESTAMP WHERE UserId = 121; INSERT INTO StaffRoles (StaffUserId, RoleId) SELECT 121, Id FROM Roles WHERE Code = 'UserAdmin'; UPDATE Users SET Status = 'active' WHERE Id = 121`);

        await connection.query(`INSERT INTO CommunitySessions (Id, CommunityUserId, ExpiresAt) VALUES ('00000000-0000-4000-8000-000000000120', 120, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 1 DAY)); INSERT INTO StaffSessions (Id, StaffUserId, WebAuthnCredentialId, MfaVerifiedAt, IpAddress, UserAgent, ExpiresAt) VALUES ('00000000-0000-4000-8000-000000000121', 121, 'credential-121', CURRENT_TIMESTAMP, '192.0.2.121', 'Recipe Shelter schema test', DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 8 HOUR))`);

        await assert.rejects(() => connection.query(`INSERT INTO CommunitySessions (Id, CommunityUserId, ExpiresAt) VALUES ('00000000-0000-4000-8000-000000000122', 121, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 1 DAY))`));
        await assert.rejects(() => connection.query(`INSERT INTO StaffSessions (Id, StaffUserId, WebAuthnCredentialId, MfaVerifiedAt, ExpiresAt) VALUES ('00000000-0000-4000-8000-000000000123', 120, 'credential-121', CURRENT_TIMESTAMP, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 8 HOUR))`));
        await assert.rejects(() => connection.query(`INSERT INTO StaffSessions (Id, StaffUserId, WebAuthnCredentialId, MfaVerifiedAt, ExpiresAt) VALUES ('00000000-0000-4000-8000-000000000124', 121, 'unknown-credential', CURRENT_TIMESTAMP, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 8 HOUR))`));
        await assert.rejects(() => connection.query(`INSERT INTO StaffSessions (Id, StaffUserId, WebAuthnCredentialId, MfaVerifiedAt, ExpiresAt) VALUES ('00000000-0000-4000-8000-000000000125', 121, 'credential-121', NULL, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 8 HOUR))`));
        await assert.rejects(() => connection.query(`UPDATE StaffSessions SET RevokedByStaffUserId = 121, RevocationType = 'self' WHERE Id = '00000000-0000-4000-8000-000000000121'`));
        ...
        const [revokedSession] = await connection.query(`SELECT IpAddress, UserAgent, RevokedByStaffUserId, RevocationType FROM StaffSessions WHERE Id = '00000000-0000-4000-8000-000000000121'`);
    ...
    it('completes enrollment and authentication atomically through the WebAuthn repository', async () => {
        ...
        await connection.query(`INSERT INTO Users (Id, Mail, Username, Password, AccountType, Status) VALUES (130, 'webauthn-staff@test.local', 'webauthn-staff', NULL, 'staff', 'inactive')`);
        await connection.query(`INSERT INTO StaffInvitations (StaffUserId, TokenHash, ExpiresAt) VALUES (130, ?, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL 10 MINUTE))`, [tokenHash]);
        ...
        const [pendingRows] = await connection.query(`SELECT u.Password, sp.Status AS StaffStatus, sp.MfaEnrolledAt, si.UsedAt, (SELECT COUNT(*) FROM StaffWebAuthnCredentials WHERE StaffUserId = 130) AS CredentialCount FROM Users AS u INNER JOIN StaffProfiles AS sp ON sp.UserId = u.Id INNER JOIN StaffInvitations AS si ON si.StaffUserId = u.Id WHERE u.Id = 130`);
        ...
        const [enrolledRows] = await connection.query(`SELECT u.Password, u.Status AS UserStatus, sp.Status AS StaffStatus, sp.MfaEnrolledAt, si.UsedAt FROM Users AS u INNER JOIN StaffProfiles AS sp ON sp.UserId = u.Id INNER JOIN StaffInvitations AS si ON si.StaffUserId = u.Id WHERE u.Id = 130`);
        const enrolled = (
