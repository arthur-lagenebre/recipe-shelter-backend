...
describe('staff privilege change requests MySQL integration', { skip: !mysqlEnabled && 'Set TEST_DB_NAME to run isolated MySQL tests' }, () => {
  ...
  it('applies the final schema then the central seed without activating the workflow', async () => {
    ...
    const [seededRequests] = await connection.query(
      `SELECT COUNT(*) AS RequestCount FROM StaffPrivilegeChangeRequests`
    );
  ...
  it('supports requested, approved and rejected records without applying role changes', async () => {
    const [approvedInsert] = await connection.execute<mysql.ResultSetHeader>(
      ...
      [
        ...
        superAdminRoleId,
        requesterUserId,
        'Prepare a two-person review without revoking access yet.'
      ]
    );
    ...
    const [rejectedInsert] = await connection.execute<mysql.ResultSetHeader>(
      ...
      [
        ...
        superAdminRoleId,
        requesterUserId,
        'Prepare a second decision state for rejection coverage.'
      ]
    );
    await connection.execute(
      `UPDATE StaffPrivilegeChangeRequests
      ...
      [
        reviewerUserId,
        'The independent reviewer rejects this prepared request.',
        rejectedInsert.insertId
      ]
    );
  ...
  it('rejects self-escalation and same-person review attempts', async () => {
    await assert.rejects(() => connection.execute(
      ...
      [
        ...
        superAdminRoleId,
        requesterUserId,
        'A staff identity must not request its own privilege escalation.'
      ]
    ));
  ...
  it('rejects inconsistent lifecycle states and invalid staff references', async () => {
    await assert.rejects(() => connection.execute(
      ...
      [
        ...
        superAdminRoleId,
        requesterUserId,
        'Approval metadata is mandatory for a final decision.'
      ]
    ));
    await assert.rejects(() => connection.execute(
      `INSERT INTO StaffPrivilegeChangeRequests
      ...
      [
        ...
        superAdminRoleId,
        999_999,
        'Every request actor must reference a staff identity.'
      ]
    ));
    await assert.rejects(() => connection.execute(
      `INSERT INTO StaffPrivilegeChangeRequests
