...
describe('automatic staff session revocation MySQL integration', { skip: !mysqlEnabled && 'Set TEST_DB_NAME to run isolated MySQL tests' }, () => {
  ...
  it('revokes only the matching realm after a password change or reset', async () => {
    ...
    await pool.execute(
      `UPDATE Users SET Password = 'new-staff-password-hash' WHERE Id = ?`,
      [staff.userId]
    );
  ...
  it('revokes all sessions immediately after disablement, lock or MFA reset', async () => {
    ...
    await pool.execute(
      `UPDATE StaffProfiles SET Status = 'locked' WHERE UserId = ?`,
      [locked.userId]
    );
    await pool.execute(
      `UPDATE StaffProfiles
