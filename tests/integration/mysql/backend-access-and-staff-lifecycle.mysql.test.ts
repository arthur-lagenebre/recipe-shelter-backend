...
describe('backend access matrix and staff lifecycle integration', { skip: !mysqlEnabled && 'Set TEST_DB_NAME to run isolated MySQL tests' }, () => {
  ...
  before(async () => {
    ...
    staleActorCookie = (await issueStaffSession(
  ...
  it('applies schema then the central seed and enforces every seeded role across every protected endpoint', async () => {
    ...
    for (const roleCode of Object.keys(STAFF_FIXTURE_IDS) as SeededRoleCode[]) {
      ...
      for (const endpoint of ACCESS_ENDPOINTS) {
        const expectedAllowed = endpoint.requiredPermissions.every((permission) =>
  ...
  it('denies identities outside the matrix and rejects disabled or role-revoked sessions immediately', async () => {
    ...
    const actual = rows.map((row) => ({
      ...
      revokedByStaffUserId: row.RevokedByStaffUserId === null
        ? null
        : Number(row.RevokedByStaffUserId)
    }));
    const expected = [
      {
  ...
  it('runs the complete staff lifecycle and applies every global or targeted revocation immediately', async () => {
    ...
    assert.equal(await sessions.createStaffSession({
      id: invitedSessionId,
    ...
    const invalidInvitedDisable = await lifecycleRequest(
      `/api/v1/admin/staff/${invitation.staffUserId}/disable`,
      {
        method: 'POST',
        cookie: actorCookie,
    ...
    assert.equal(await staffMfa.saveChallenge({
      id: challengeId,
    ...
    const crossRoleRead = await lifecycleRequest(
      `/api/v1/admin/staff/${invitation.staffUserId}`,
    ...
    const targetDetails = await lifecycleRequest(
      `/api/v1/admin/staff/${invitation.staffUserId}`,
      { cookie: actorCookie }
    );
    ...
    const grantUserAdmin = await lifecycleRequest(
      `/api/v1/admin/staff/${invitation.staffUserId}/roles/UserAdmin`,
      {
        method: 'POST',
        cookie: actorCookie,
    ...
    const revokeRecipeModerator = await lifecycleRequest(
      `/api/v1/admin/staff/${invitation.staffUserId}/roles/RecipeModerator`,
      {
        method: 'DELETE',
        cookie: actorCookie,
    ...
    const revokeLastRole = await lifecycleRequest(
      `/api/v1/admin/staff/${invitation.staffUserId}/roles/UserAdmin`,
      {
        method: 'DELETE',
        cookie: actorCookie,
    ...
    assert.deepEqual(lastRoleRevocations.map((row) => row.RevocationType), [
      'roles_removed',
      'roles_removed'
    ]);

    const restoreRecipeModerator = await lifecycleRequest(
      `/api/v1/admin/staff/${invitation.staffUserId}/roles/RecipeModerator`,
      {
        method: 'POST',
        cookie: actorCookie,
    ...
    const staleDisable = await lifecycleRequest(
      `/api/v1/admin/staff/${invitation.staffUserId}/disable`,
      {
        method: 'POST',
    ...
    const disable = await lifecycleRequest(
      `/api/v1/admin/staff/${invitation.staffUserId}/disable`,
      {
        method: 'POST',
        cookie: actorCookie,
    ...
    const duplicateDisable = await lifecycleRequest(
      `/api/v1/admin/staff/${invitation.staffUserId}/disable`,
      {
        method: 'POST',
        cookie: actorCookie,
    ...
    const enable = await lifecycleRequest(
      `/api/v1/admin/staff/${invitation.staffUserId}/enable`,
      {
        method: 'POST',
        cookie: actorCookie,
    ...
    const duplicateEnable = await lifecycleRequest(
      `/api/v1/admin/staff/${invitation.staffUserId}/enable`,
      {
        method: 'POST',
        cookie: actorCookie,
    ...
    const managedSessions = await lifecycleRequest(
      `/api/v1/admin/staff/${invitation.staffUserId}/sessions`,
      { cookie: actorCookie }
    );
    ...
    const ownRevocation = await lifecycleRequest(
      `/api/v1/admin/auth/sessions/${postEnableSessionTwo.id}`,
      { method: 'DELETE', cookie: postEnableSessionTwo.cookie }
    );
    ...
    const [auditRows] = await pool.execute<RowDataPacket[]>(
      `SELECT Action FROM AdminAuditLogs ORDER BY Id`
    );
  ...
  it('returns stable staff business errors without mutation or audit side effects', async () => {
    ...
    const invalidReason = await lifecycleRequest(
      `/api/v1/admin/staff/${target.id}/roles/UserAdmin`,
      {
        method: 'POST',
        cookie: actorCookie,
        body: { reason: 'short' }
    ...
    assert.deepEqual((await staff.findById(target.id))?.roles.map((role) => role.code), [
      'RecipeModerator'
    ]);
    assert.equal(await countAuditRows(), auditCountBefore);
  });

  async function createActiveStaffFixture(
    staffUserId: number,
    roleCode: SeededRoleCode | null
  ): Promise<void> {
    await pool.execute(
      `INSERT INTO Users (Id, Mail, Username, Password, AccountType, Status, EmailValidatedAt)
    ...
    await pool.execute(
      `UPDATE StaffProfiles SET MfaEnrolledAt = CURRENT_TIMESTAMP WHERE UserId = ?`,
      [staffUserId]
    );
    if (roleCode) {
      await pool.execute(
  ...
  async function findStaffSessionVersion(staffUserId: number): Promise<number> {
    const [rows] = await pool.execute<RowDataPacket[]>(
      `SELECT SessionVersion FROM StaffProfiles WHERE UserId = ?`,
      [staffUserId]
    );
  ...
  function createStaffLifecycleApp() {
    ...
    const staffSessionsController = createStaffSessionsController(
      new StaffSessionService(sessions, users, auditActions)
    );
    const invitationController = createStaffInvitationsController(new StaffInvitationService(
  ...
  async function countAuditRows(): Promise<number> {
    const [rows] = await pool.execute<RowDataPacket[]>(
      `SELECT COUNT(*) AS AuditCount FROM AdminAuditLogs`
    );
