...
describe('catalog proposals schema MySQL integration', { skip: !mysqlEnabled && 'Set TEST_DB_NAME to run isolated MySQL tests' }, () => {
    ...
    it('builds the final model from an empty database then applies the central seed', async () => {
       ...
       ORDER BY ORDINAL_POSITION`,
        ...
        assert.deepEqual(columns, [
            ...
            {
                ColumnName: 'Status',
                ColumnType: "enum('pending','accepted','rejected','merged')",
                IsNullable: 'NO',
    ...
    it('creates community proposals through the service and MySQL repository with duplicate validation', async () => {
        const service = new CatalogProposalService(new CatalogProposalRepositoryMysql(connection as unknown as Pool));
    ...
    it('processes the staff queue with canonical, association, alias and rejection audits in atomic transactions', async () => {
        ...
        const acceptedTag = await service.acceptTag(
            proposalIds.acceptedTag,
            {
                groupId: 8_600,
        ...
        const acceptedIngredient = await service.acceptIngredient(
            proposalIds.acceptedIngredient,
            {
        ...
        await service.convertIngredientToAlias(
            proposalIds.aliasedIngredient,
            {
       ...
       ORDER BY Id`,
        ...
        assert.deepEqual(reviews, [
            ...
            {
                Id: proposalIds.acceptedIngredient,
                Status: 'accepted',
                MatchedTagId: null,
            ...
            {
                Id: proposalIds.aliasedIngredient,
                Status: 'merged',
                MatchedTagId: null,
                MatchedIngredientId: activeIngredientId,
                ReviewedByStaffUserId: reviewerUserId
