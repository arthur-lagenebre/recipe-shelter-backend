...
describe('tag catalog schema integration', { skip: !mysqlEnabled && 'Set TEST_DB_NAME to run isolated MySQL tests' }, () => {
    ...
    it('builds the final tag model from an empty database and the central seed', async () => {
        const databaseName = requireTagCatalogTestDatabaseName();
        const [columns] = await connection.query(`SELECT COLUMN_NAME AS ColumnName, COLUMN_TYPE AS ColumnType, IS_NULLABLE AS IsNullable, COLUMN_DEFAULT AS ColumnDefault FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'Tags' ORDER BY ORDINAL_POSITION`, [databaseName]);
        ...
        const [indexes] = await connection.query(`SELECT DISTINCT INDEX_NAME AS IndexName FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'Tags' ORDER BY INDEX_NAME`, [databaseName]);
        assert.deepEqual(
        ...
        const [tagIntegrityTriggers] = await connection.query(`SELECT TRIGGER_NAME AS TriggerName, LOWER(EVENT_OBJECT_TABLE) AS TableName, ACTION_TIMING AS ActionTiming, EVENT_MANIPULATION AS EventManipulation FROM information_schema.TRIGGERS WHERE TRIGGER_SCHEMA = ? AND TRIGGER_NAME IN ('recipe_tags_active_tag_BI', 'tags_merged_recipe_associations_BU', 'tags_no_delete_BD') ORDER BY TRIGGER_NAME`, [databaseName]);
    ...
    it('rejects equivalent active names while preserving deprecated and merged variants', async () => {
        await connection.query(`INSERT INTO Tags (Id, GroupId, Name, NormalizedName, Slug, Description) VALUES (910, 1, 'Crème brûlée', 'creme brulee', 'normalization-canonical', 'Canonical fixture')`);
        ...
        for (const { name, slug } of activeVariants) {
            await assert.rejects(() => connection.query(`INSERT INTO Tags (GroupId, Name, NormalizedName, Slug) VALUES (1, ?, 'creme brulee', ?)`, [name, slug]), assertActiveNormalizedDuplicateRejected);
        }

        await assert.rejects(() => connection.query(`INSERT INTO Tags (GroupId, Name, NormalizedName, Slug, Status) VALUES (1, 'Crème brûlée', 'unrelated value', 'normalization-mismatch', 'deprecated')`));

        await connection.query(`INSERT INTO Tags (Id, GroupId, Name, NormalizedName, Slug, Status) VALUES (911, 1, 'Creme---brulee', 'creme brulee', 'normalization-deprecated', 'deprecated'); INSERT INTO Tags (Id, GroupId, Name, NormalizedName, Slug, Status, MergedIntoTagId) VALUES (912, 1, 'CRÈME BRÛLÉE', 'creme brulee', 'normalization-merged', 'merged', 910)`);

        await assert.rejects(() => connection.query(`UPDATE Tags SET Status = 'active' WHERE Id = 911`), assertActiveNormalizedDuplicateRejected);

        const [historicalVariants] = await connection.query(`SELECT Id, Status, MergedIntoTagId FROM Tags WHERE Id IN (911, 912) ORDER BY Id`);
    ...
    it('enforces canonical identity, lifecycle coherence, merge references and non-destructive retention', async () => {
        await connection.query(`INSERT INTO Tags (Id, GroupId, Name, NormalizedName, Slug, Description, Status) VALUES (900, 1, 'Canonical fixture', 'canonical fixture', 'canonical-fixture', 'Canonical target', 'active'), (901, 1, 'Deprecated fixture', 'deprecated fixture', 'deprecated-fixture', NULL, 'deprecated'); INSERT INTO Tags (Id, GroupId, Name, NormalizedName, Slug, Status, MergedIntoTagId) VALUES (902, 1, 'Merged fixture', 'merged fixture', 'merged-fixture', 'merged', 900)`);
        ...
        await connection.query(`UPDATE Tags SET Description = 'Updated canonical target' WHERE Id = 900`);
        ...
        await assert.rejects(() => connection.query(`INSERT INTO Tags (GroupId, Name, NormalizedName, Slug, Status) VALUES (1, 'Missing merge target', 'missing merge target', 'missing-merge-target', 'merged')`));
        await assert.rejects(() => connection.query(`INSERT INTO Tags (GroupId, Name, NormalizedName, Slug, Status, MergedIntoTagId) VALUES (1, 'Unexpected merge target', 'unexpected merge target', 'unexpected-merge-target', 'active', 900)`));
        await assert.rejects(() => connection.query(`INSERT INTO Tags (Id, GroupId, Name, NormalizedName, Slug, Status, MergedIntoTagId) VALUES (903, 1, 'Self merge', 'self merge', 'self-merge', 'merged', 903)`));
        await assert.rejects(() => connection.query(`INSERT INTO Tags (GroupId, Name, NormalizedName, Slug, Status, MergedIntoTagId) VALUES (1, 'Unknown merge target', 'unknown merge target', 'unknown-merge-target', 'merged', 999999)`));
        await assert.rejects(() => connection.query(`INSERT INTO Tags (GroupId, Name, NormalizedName, Slug, Status, MergedIntoTagId) VALUES (1, 'Deprecated merge target', 'deprecated merge target', 'deprecated-merge-target', 'merged', 901)`));
        await assert.rejects(() => connection.query(`UPDATE Tags SET Status = 'deprecated' WHERE Id = 900`));
        await assert.rejects(() => connection.query(`INSERT INTO Tags (GroupId, Name, NormalizedName, Slug) VALUES (1, 'Mismatched normalized name', 'another value', 'mismatched-normalized-name')`));
        await assert.rejects(() => connection.query(`INSERT INTO Tags (GroupId, Name, NormalizedName, Slug) VALUES (1, 'Uppercase normalized name', 'UPPERCASE FIXTURE', 'uppercase-fixture')`));
        await assert.rejects(() => connection.query(`INSERT INTO Tags (GroupId, Name, NormalizedName, Slug) VALUES (1, 'Invalid slug', 'invalid slug', 'Invalid Slug')`));
        await assert.rejects(() => connection.query(`INSERT INTO Tags (GroupId, Name, NormalizedName, Slug, Description) VALUES (1, 'Blank description', 'blank description', 'blank-description', '   ')`));
    ...
    it('persists paginated administration writes and merges recipe relationships without deleting tags', async () => {
        ...
        try {
            ...
            await db.execute(`INSERT INTO Users (Id, Mail, Username, Password, AccountType, Status, EmailValidatedAt) VALUES (930, 'tag-admin-fixture@example.test', 'tag-admin-fixture', 'test-password-hash', 'community', 'active', CURRENT_TIMESTAMP)`);
            await db.execute(`INSERT INTO Recipes (Id, UserId, CategoryId, Title, Slug, Description, PrepTimeMinutes, Servings) VALUES (930, 930, 1, 'Tag merge fixture one', 'tag-merge-fixture-one', 'Fixture', 5, 2), (931, 930, 1, 'Tag merge fixture two', 'tag-merge-fixture-two', 'Fixture', 5, 2)`);
            await db.execute(`INSERT INTO Tags (Id, GroupId, Name, NormalizedName, Slug) VALUES (930, 1, 'Merge source fixture', 'merge source fixture', 'merge-source-fixture'), (931, 1, 'Merge target fixture', 'merge target fixture', 'merge-target-fixture')`);
            await db.execute(`INSERT INTO Tags (Id, GroupId, Name, NormalizedName, Slug, Status, MergedIntoTagId) VALUES (932, 1, 'Previous alias fixture', 'previous alias fixture', 'previous-alias-fixture', 'merged', 930)`);
            await db.execute(`INSERT INTO RecipeTags (RecipeId, TagId) VALUES (930, 930), (931, 930), (931, 931)`);
            ...
            const [recipeTags] = await db.query(`SELECT RecipeId, TagId FROM RecipeTags WHERE RecipeId IN (930, 931) ORDER BY RecipeId, TagId`);
            assert.deepEqual(recipeTags, [
                { RecipeId: 930, TagId: 931 },
                { RecipeId: 931, TagId: 931 }
            ]);

            const [mergedTags] = await db.query(`SELECT Id, Status, MergedIntoTagId FROM Tags WHERE Id IN (930, 932) ORDER BY Id`);
            assert.deepEqual(mergedTags, [
                { Id: 930, Status: 'merged', MergedIntoTagId: 931 },
                { Id: 932, Status: 'merged', MergedIntoTagId: 931 }
            ]);

            await assert.rejects(() => db.execute(`INSERT INTO RecipeTags (RecipeId, TagId) VALUES (930, 930)`), /Recipes can only reference active canonical tags/);

            await db.execute(`INSERT INTO Tags (Id, GroupId, Name, NormalizedName, Slug) VALUES (933, 1, 'Still associated fixture', 'still associated fixture', 'still-associated-fixture')`);
            await db.execute(`INSERT INTO RecipeTags (RecipeId, TagId) VALUES (930, 933)`);
            await assert.rejects(() => db.execute(`DELETE FROM Tags WHERE Id = 933`), assertReferencedTagDeletionRejected);

            const [retainedActiveTag] = await db.query(`SELECT tags.Id, tags.Status, COUNT(recipe_tags.RecipeId) AS RecipeCount FROM Tags AS tags LEFT JOIN RecipeTags AS recipe_tags ON recipe_tags.TagId = tags.Id WHERE tags.Id = 933 GROUP BY tags.Id, tags.Status`);
            ...
            const [retainedDeprecatedTag] = await db.query(`SELECT tags.Id, tags.Status, COUNT(recipe_tags.RecipeId) AS RecipeCount FROM Tags AS tags LEFT JOIN RecipeTags AS recipe_tags ON recipe_tags.TagId = tags.Id WHERE tags.Id = 933 GROUP BY tags.Id, tags.Status`);
            ...
            await assert.rejects(() => db.execute(`UPDATE Tags SET Status = 'merged', MergedIntoTagId = 931 WHERE Id = 933`), /A tag must have no recipe associations before being merged/);
        } finally {
            await db.rollback();
            db.release();
        }
    ...
    it('commits the merge and its before/after audit atomically, then rolls both back on audit failure', async () => {
        await connection.query(`INSERT INTO Users (Id, Mail, Username, Password, AccountType, Status) VALUES (950, 'tag-merge-staff@example.test', 'tag-merge-staff', 'test-password-hash', 'staff', 'inactive'), (951, 'tag-merge-author@example.test', 'tag-merge-author', 'test-password-hash', 'community', 'active'); INSERT INTO Recipes (Id, UserId, CategoryId, Title, Slug, Description, PrepTimeMinutes, Servings) VALUES (950, 951, 1, 'Transactional merge fixture one', 'transactional-merge-fixture-one', 'Fixture', 5, 2), (951, 951, 1, 'Transactional merge fixture two', 'transactional-merge-fixture-two', 'Fixture', 5, 2); INSERT INTO Tags (Id, GroupId, Name, NormalizedName, Slug) VALUES (950, 1, 'Transactional source fixture', 'transactional source fixture', 'transactional-source-fixture'), (951, 1, 'Transactional target fixture', 'transactional target fixture', 'transactional-target-fixture'), (953, 1, 'Rollback source fixture', 'rollback source fixture', 'rollback-source-fixture'), (954, 1, 'Rollback target fixture', 'rollback target fixture', 'rollback-target-fixture'); INSERT INTO Tags (Id, GroupId, Name, NormalizedName, Slug, Status, MergedIntoTagId) VALUES (952, 1, 'Transactional alias fixture', 'transactional alias fixture', 'transactional-alias-fixture', 'merged', 950); INSERT INTO RecipeTags (RecipeId, TagId) VALUES (950, 950), (951, 950), (951, 951), (950, 953)`);

        const [targetBeforeRows] = await connection.query(`SELECT Id, GroupId, Name, NormalizedName, Slug, Description, Status, MergedIntoTagId, CreatedAt, UpdatedAt FROM Tags WHERE Id = 951`);
        ...
        const [targetAfterRows] = await connection.query(`SELECT Id, GroupId, Name, NormalizedName, Slug, Description, Status, MergedIntoTagId, CreatedAt, UpdatedAt FROM Tags WHERE Id = 951`);
        assert.deepEqual((targetAfterRows as Array<Record<string, unknown>>)[0], targetBefore);

        const [committedRecipeTags] = await connection.query(`SELECT RecipeId, TagId FROM RecipeTags WHERE RecipeId IN (950, 951) AND TagId IN (950, 951) ORDER BY RecipeId, TagId`);
        assert.deepEqual(committedRecipeTags, [
            { RecipeId: 950, TagId: 951 },
            { RecipeId: 951, TagId: 951 }
        ]);

        const [committedTags] = await connection.query(`SELECT Id, Status, MergedIntoTagId FROM Tags WHERE Id IN (950, 951, 952) ORDER BY Id`);
        ...
        const [auditRows] = await connection.query(`SELECT Action, TargetType, TargetId, Reason, JSON_UNQUOTE(JSON_EXTRACT(BeforeValues, '$.source.status')) AS BeforeSourceStatus, CAST(JSON_UNQUOTE(JSON_EXTRACT(BeforeValues, '$.recipeAssociations.sourceCount')) AS UNSIGNED) AS BeforeSourceRecipeCount, CAST(JSON_UNQUOTE(JSON_EXTRACT(BeforeValues, '$.recipeAssociations.targetCount')) AS UNSIGNED) AS BeforeTargetRecipeCount, CAST(JSON_UNQUOTE(JSON_EXTRACT(BeforeValues, '$.recipeAssociations.sharedCount')) AS UNSIGNED) AS BeforeSharedRecipeCount, JSON_UNQUOTE(JSON_EXTRACT(AfterValues, '$.source.status')) AS AfterSourceStatus, CAST(JSON_UNQUOTE(JSON_EXTRACT(AfterValues, '$.recipeAssociations.sourceCount')) AS UNSIGNED) AS AfterSourceRecipeCount, CAST(JSON_UNQUOTE(JSON_EXTRACT(AfterValues, '$.recipeAssociations.targetCount')) AS UNSIGNED) AS AfterTargetRecipeCount, CAST(JSON_UNQUOTE(JSON_EXTRACT(AfterValues, '$.transfer.transferredRecipeCount')) AS UNSIGNED) AS TransferredRecipeCount, CAST(JSON_UNQUOTE(JSON_EXTRACT(AfterValues, '$.transfer.deduplicatedRecipeCount')) AS UNSIGNED) AS DeduplicatedRecipeCount, CAST(JSON_UNQUOTE(JSON_EXTRACT(AfterValues, '$.transfer.redirectedMergedTagCount')) AS UNSIGNED) AS RedirectedMergedTagCount FROM AdminAuditLogs WHERE CorrelationId = ?`, [correlationId]);
        ...
        const [rolledBack] = await connection.query(`SELECT source.Status AS SourceStatus, source.MergedIntoTagId, (SELECT COUNT(*) FROM RecipeTags WHERE TagId = 953) AS SourceRecipeCount, (SELECT COUNT(*) FROM RecipeTags WHERE TagId = 954) AS TargetRecipeCount, (SELECT COUNT(*) FROM AdminAuditLogs WHERE CorrelationId = ?) AS AuditCount FROM Tags AS source WHERE source.Id = 953`, [failedCorrelationId]);
    ...
    it('serializes a concurrent recipe association so it cannot recreate a merged source link', async () => {
        await connection.query(`INSERT INTO Tags (Id, GroupId, Name, NormalizedName, Slug) VALUES (960, 1, 'Concurrent source fixture', 'concurrent source fixture', 'concurrent-source-fixture'), (961, 1, 'Concurrent target fixture', 'concurrent target fixture', 'concurrent-target-fixture')`);
        ...
        try {
            ...
            const pendingAssociation = recipeDb
                .execute(`INSERT INTO RecipeTags (RecipeId, TagId) VALUES (950, 960)`)
            ...
            const [persistedState] = await connection.query(`SELECT source.Status AS SourceStatus, source.MergedIntoTagId, (SELECT COUNT(*) FROM RecipeTags WHERE TagId = 960) AS SourceRecipeCount, (SELECT COUNT(*) FROM RecipeTags WHERE TagId = 961) AS TargetRecipeCount FROM Tags AS source WHERE source.Id = 960`);
