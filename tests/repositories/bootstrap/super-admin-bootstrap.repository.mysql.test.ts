...
describe('SuperAdminBootstrapRepositoryMysql', () => {
    it('locks the role and atomically creates the invited account, role and expiring MFA invitation', async () => {
        ...
        assert.deepEqual(
            await repository.createFirst(input, async ({ userId }) => {
    ...
    it('stops before account creation when any SuperAdmin already exists', async () => {
        for (const status of ['active', 'invited']) {
            ...
            assert.deepEqual(await repository.createFirst(input, completeCreation), {
                status: 'super_admin_exists',
                active: status === 'active'
            });
            assert.equal(
                fake.statements.some(({ sql }) => /INSERT INTO Users/.test(sql)),
                false
            );
            assert.deepEqual(fake.counts(), { commits: 1, rollbacks: 0, releases: 1 });
        }
    ...
    it('continues when the locked role aggregate confirms that no SuperAdmin exists', async () => {
        ...
        assert.deepEqual(await new SuperAdminBootstrapRepositoryMysql(fake.pool).createFirst(input, completeCreation), {
            status: 'created',
            userId: 42
        });
        assert.equal(
    ...
    it('requires the central seed role and rejects identities already in use', async () => {
        ...
        assert.deepEqual(await new SuperAdminBootstrapRepositoryMysql(missingRole.pool).createFirst(input, completeCreation), {
            status: 'role_missing'
        });
        ...
        assert.deepEqual(await new SuperAdminBootstrapRepositoryMysql(emailTaken.pool).createFirst(input, completeCreation), {
            status: 'email_taken'
        });
        ...
        assert.deepEqual(await new SuperAdminBootstrapRepositoryMysql(usernameTaken.pool).createFirst(input, completeCreation), {
    ...
    it('rolls back uncommitted creation when invitation delivery fails without deleting a staff identity', async () => {
        ...
        await assert.rejects(
            () =>
                repository.createFirst(input, async () => {
                    throw deliveryError;
                }),
            deliveryError
        );
        assert.equal(
            fake.statements.some(({ sql }) => /DELETE\s+FROM/i.test(sql)),
            false
        );
    ...
    it('rolls back, releases and preserves unexpected database errors', async () => {
        ...
        await assert.rejects(() => new SuperAdminBootstrapRepositoryMysql(create.pool).createFirst(input, completeCreation), createError);
