...
describe('CatalogProposalRepositoryMysql', () => {
    ...
    it('maps only the pending proposal unique constraint to a duplicate outcome', async () => {
        ...
        const duplicateRepository = new CatalogProposalRepositoryMysql(
            createPool([], [{ error: duplicateError('catalog_proposals_pending_recipe_name_UK') }])
        );
    ...
    it('locks and atomically reviews a pending proposal', async () => {
        ...
        assert.deepEqual(
            ...
            mapCatalogProposal(reviewedRow)
        );
