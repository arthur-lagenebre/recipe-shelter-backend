...
describe('StaffInvitationRepositoryMysql', () => {
  ...
  it('distinguishes an existing invitation from another existing email', async () => {
    ...
    assert.deepEqual(
      await new StaffInvitationRepositoryMysql(existingInvitation.pool).create(input),
      { status: 'invitation_exists', invitationId: 7 }
    );
    assert.equal(existingInvitation.statements.some(({ sql }) => /INSERT INTO Users/.test(sql)), false);
    ...
    assert.deepEqual(
      await new StaffInvitationRepositoryMysql(emailTaken.pool).create(input),
      { status: 'email_taken' }
    );
  });
  ...
  it('rejects display-name conflicts and missing role codes before account creation', async () => {
    ...
    assert.deepEqual(
      await new StaffInvitationRepositoryMysql(displayNameTaken.pool).create(input),
    ...
    assert.equal(missingRole.statements.some(({ sql }) => /FROM Users/.test(sql)), false);
  });
  ...
  it('maps concurrent unique identity conflicts and rolls back its own transaction', async () => {
    ...
    assert.deepEqual(
      await new StaffInvitationRepositoryMysql(fake.pool).create(input),
      { status: 'email_taken' }
    );
  ...
  it('rolls back and preserves unexpected persistence errors', async () => {
    ...
    await assert.rejects(
      () => new StaffInvitationRepositoryMysql(fake.pool).create(input),
      persistenceError
    );
    assert.deepEqual(fake.counts(), { commits: 0, rollbacks: 1, releases: 1 });
  });
