...
describe('AdminTagRepositoryMysql catalog operations', () => {
    ...
    it('creates and updates tags, including the defensive reload failures', async () => {
        ...
        await assert.rejects(() => repository.update({ id: 999, ...writeInput }, missingUpdateDb), /Tag updated but cannot be reloaded/);
    });
    ...
    it('maps canonical duplicate conflicts and preserves unexpected write errors', async () => {
        ...
        assert.deepEqual(await repository.create(writeInput, createConnection([], [throwResponse(normalizedDuplicate)])), {
            status: 'normalized_name_taken'
        });

        const slugDuplicate = duplicateError('tags_slug_UK');
        assert.deepEqual(await repository.update({ id: 42, ...writeInput }, createConnection([], [throwResponse(slugDuplicate)])), {
            status: 'slug_taken'
        });
...
describe('AdminTagRepositoryMysql merge', () => {
    ...
    it('fails closed when locked association counts no longer match the writes', async () => {
        ...
        await assert.rejects(() => repository.merge(10, 20, db), /Tag recipe associations changed during merge/);
