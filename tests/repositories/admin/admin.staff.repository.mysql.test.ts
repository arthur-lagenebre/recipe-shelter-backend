...
describe('AdminStaffRepositoryMysql defensive branches', () => {
  ...
  it('maps staff roles and defaults to an empty role list when none is assigned', async () => {
    ...
    assert.deepEqual(accounts[0]?.roles.map(({ code }) => code), ['SuperAdmin', 'CatalogManager']);
  ...
  it('fails closed when the SuperAdmin role or specialized moderation audit link is missing', async () => {
    ...
    assert.equal(
      await repository.lockAndCheckLastActiveSuperAdmin(42, createConnection([], [[[], []]])),
      false
    );

    await assert.rejects(
      () => repository.createModerationLog(
        100,
        42,
        createConnection([], [[{ affectedRows: 0 }, []]])
      ),
    ...
    await repository.createModerationLog(
      100,
      42,
      createConnection([], [[{ affectedRows: 1 }, []]])
    );
  });
  ...
  it('defaults a missing active-session count to zero and reports mutation no-ops', async () => {
    ...
    assert.equal(
      await repository.disable(42, 43, 'Coverage reason is sufficiently detailed.', successfulDisable),
      0
    );
    ...
    assert.equal(
      await repository.disable(42, 43, 'Coverage reason is sufficiently detailed.', concurrentDisable),
      null
    );
