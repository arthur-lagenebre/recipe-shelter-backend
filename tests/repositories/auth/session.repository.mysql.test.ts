...
describe('SessionRepositoryMysql', () => {
  it('persists community and MFA-backed staff sessions in separate tables', async () => {
    ...
    await repository.createStaffSession({ id: 'staff-id', userId: 1, expiresAt, mfaVerifiedAt });
    ...
    assert.match(fake.statements[1]?.sql ?? '', /INSERT INTO StaffSessions[\s\S]+MfaVerifiedAt/);
    assert.deepEqual(fake.statements[1]?.params, ['staff-id', 1, mfaVerifiedAt, expiresAt]);
  });
  ...
  it('checks expiry, revocation and MFA for the matching session realm', async () => {
    ...
    assert.match(fake.statements[1]?.sql ?? '', /FROM StaffSessions[\s\S]+MfaVerifiedAt IS NOT NULL/);
  });
