...
describe('staff invitation DTO', () => {
  it('normalizes the email, display name and initial role codes', () => {
    assert.deepEqual(parseCreateStaffInvitationBody({
  ...
  it('rejects malformed bodies and identities', () => {
    const cases: Array<{ body: unknown; code: string }> = [
      ...
      { body: { email: 'staff@example.com', displayName: '  ', roles: ['UserAdmin'] }, code: 'STAFF_INVITATION_DISPLAY_NAME_REQUIRED' },
      { body: { email: 'staff@example.com', displayName: 'ab', roles: ['UserAdmin'] }, code: 'STAFF_INVITATION_DISPLAY_NAME_TOO_SHORT' },
      { body: { email: 'staff@example.com', displayName: 'a'.repeat(65), roles: ['UserAdmin'] }, code: 'STAFF_INVITATION_DISPLAY_NAME_TOO_LONG' }
    ];
  ...
  it('requires a bounded, non-empty and unique role code list', () => {
    ...
    for (const testCase of cases)
      assert.throws(() => parseCreateStaffInvitationBody({ ...base, roles: testCase.roles }), (error) => assertHttpError(error, testCase.code));
  });
});
