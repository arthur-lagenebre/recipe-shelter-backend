...
describe('admin tags DTO validation', () => {
    ...
    it('rejects malformed identifiers and filters with stable errors', () => {
        ...
        assert.throws(
            () => parseAdminTagListFilters({ status: 42 }),
            (error) => assertHttpError(error, 'ADMIN_TAGS_BAD_STATUS')
        );
        assert.throws(
            () => parseAdminTagListFilters({ groupId: '0' }),
            (error) => assertHttpError(error, 'ADMIN_TAGS_BAD_GROUP_ID')
        );
        assert.throws(
            () => parseAdminTagListFilters({ groupId: '9007199254740992' }),
            (error) => assertHttpError(error, 'ADMIN_TAGS_BAD_GROUP_ID')
        );
        assert.throws(
            () => parseAdminTagListFilters({ q: 42 }),
            (error) => assertHttpError(error, 'ADMIN_TAGS_BAD_SEARCH')
        );
        assert.throws(
            () => parseAdminTagListFilters({ q: '   ' }),
            (error) => assertHttpError(error, 'ADMIN_TAGS_BAD_SEARCH')
        );
        assert.throws(
            () => parseAdminTagListFilters({ q: 'x'.repeat(256) }),
            (error) => assertHttpError(error, 'ADMIN_TAGS_BAD_SEARCH')
        );
    });
    ...
    it('rejects empty, oversized and structurally invalid tag payloads', () => {
        ...
        assert.throws(
            () => parseCreateAdminTagBody({ groupId: 1, name: ' ' }),
            (error) => assertHttpError(error, 'ADMIN_TAGS_NAME_REQUIRED')
        );
        assert.throws(
            () => parseCreateAdminTagBody({ groupId: 1, name: 'x'.repeat(256) }),
            (error) => assertHttpError(error, 'ADMIN_TAGS_NAME_TOO_LONG')
        );
        assert.throws(
            () => parseCreateAdminTagBody({ groupId: 1, name: 'Valid', slug: 42 }),
            (error) => assertHttpError(error, 'ADMIN_TAGS_SLUG_INVALID')
        );
        assert.throws(
            () => parseCreateAdminTagBody({ groupId: 1, name: 'Valid', slug: 'x'.repeat(256) }),
            (error) => assertHttpError(error, 'ADMIN_TAGS_SLUG_INVALID')
        );
        assert.throws(
            () => parseCreateAdminTagBody({ groupId: 1, name: 'Valid', slug: 'Not Valid' }),
            (error) => assertHttpError(error, 'ADMIN_TAGS_SLUG_INVALID')
        );
        assert.throws(
            () => parseCreateAdminTagBody({ groupId: 1, name: 'Valid', description: 42 }),
        ...
        assert.throws(
            () => parseUpdateAdminTagBody({ description: 'x'.repeat(1001) }),
            (error) => assertHttpError(error, 'ADMIN_TAGS_DESCRIPTION_TOO_LONG')
        );
    });

    it('requires meaningful lifecycle and merge reasons', () => {
        assert.equal(parseAdminTagActionReasonBody({ reason: '  Tag devenu obsolète.  ' }, 'deprecate'), 'Tag devenu obsolète.');
        assert.deepEqual(
