...
import { parseAcceptEquipmentCatalogProposalBody, parseAcceptIngredientCatalogProposalBody, parseAcceptTagCatalogProposalBody, parseAdminCatalogProposalIdParam, parseAdminCatalogProposalListFilters, parseAssociateEquipmentCatalogProposalBody, parseAssociateIngredientCatalogProposalBody, parseAssociateTagCatalogProposalBody, parseConvertCatalogProposalToAliasBody, parseRejectCatalogProposalBody } from '../../../src/api/admin/admin.catalog-proposals.dto.js';
...
describe('admin.catalog-proposals.dto', () => {
    ...
    it('parses typed accept bodies without accepting a canonical name', () => {
        ...
        assert.deepEqual(
            parseAcceptEquipmentCatalogProposalBody({
                name: 'Ignored free canonical name',
                slug: 'ignored-slug',
                reason: 'Nouvel ustensile canonique validé.'
            }),
            {
                reason: 'Nouvel ustensile canonique validé.'
            }
    ...
    it('parses reject, association and alias decisions', () => {
        assert.equal(parseRejectCatalogProposalBody({ reason: 'Suggestion hors périmètre.' }), 'Suggestion hors périmètre.');
        assert.deepEqual(
        ...
        assert.deepEqual(
            parseAssociateEquipmentCatalogProposalBody({
                ...
                reason: 'Correspond à un ustensile existant.'
            }),
            {
                targetEquipmentId: 14,
                reason: 'Correspond à un ustensile existant.'
    ...
    it('rejects malformed ids, filters, metadata and mandatory reasons', () => {
        const invalidCases: Array<{ run: () => unknown; code: string }> = [
            ...
            { run: () => parseAdminCatalogProposalListFilters({ proposalType: 'unknown' }), code: 'ADMIN_CATALOG_PROPOSALS_BAD_TYPE' },
            ...
            { run: () => parseAcceptEquipmentCatalogProposalBody([]), code: 'ADMIN_CATALOG_PROPOSALS_ACCEPT_BAD_BODY' },
            { run: () => parseAcceptEquipmentCatalogProposalBody({ reason: 'court' }), code: 'ADMIN_CATALOG_PROPOSALS_ACCEPT_REASON_TOO_SHORT' },
            { run: () => parseAssociateEquipmentCatalogProposalBody({ reason: 'Identifiant cible manquant.' }), code: 'ADMIN_CATALOG_PROPOSALS_BAD_TARGET_EQUIPMENT_ID' },
