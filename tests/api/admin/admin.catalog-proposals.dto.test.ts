...
import {
    ...
    parseRejectCatalogProposalBody
} from '../../../src/api/admin/admin.catalog-proposals.dto.js';
...
describe('admin.catalog-proposals.dto', () => {
    ...
    it('rejects malformed ids, filters, metadata and mandatory reasons', () => {
        const invalidCases: Array<{ run: () => unknown; code: string }> = [
            ...
            {
                run: () => parseAcceptTagCatalogProposalBody({ groupId: 0, reason: 'Motif suffisamment long.' }),
                code: 'ADMIN_CATALOG_PROPOSALS_BAD_TAG_GROUP_ID'
            },
            {
                run: () => parseAcceptTagCatalogProposalBody({ groupId: 1, slug: 'Bad Slug', reason: 'Motif suffisamment long.' }),
                code: 'ADMIN_CATALOG_PROPOSALS_TAG_SLUG_INVALID'
            },
            {
                run: () => parseAcceptTagCatalogProposalBody({ groupId: 1, description: 42, reason: 'Motif suffisamment long.' }),
                code: 'ADMIN_CATALOG_PROPOSALS_TAG_DESCRIPTION_INVALID'
            },
            { run: () => parseRejectCatalogProposalBody({ reason: 'court' }), code: 'ADMIN_CATALOG_PROPOSALS_REJECT_REASON_TOO_SHORT' },
            {
                run: () => parseRejectCatalogProposalBody({ reason: 'x'.repeat(1001) }),
                code: 'ADMIN_CATALOG_PROPOSALS_REJECT_REASON_TOO_LONG'
            },
            {
                run: () =>
                    parseConvertCatalogProposalToAliasBody({
        ...
        for (const { run, code } of invalidCases) assert.throws(run, (error) => assertBadRequest(error, code));
    });
});
