...
describe('admin ingredients DTO validation', () => {
    ...
    it('rejects invalid canonical ingredient bodies', () => {
        ...
        assert.throws(() => parseUpdateAdminIngredientBody({ name: 'x'.repeat(256) }), matchesCode('ADMIN_INGREDIENTS_NAME_TOO_LONG'));
    });
    ...
    it('rejects invalid alias bodies', () => {
        ...
        assert.throws(() => parseUpdateAdminIngredientAliasBody({}), matchesCode('ADMIN_INGREDIENT_ALIASES_UPDATE_EMPTY'));
        assert.throws(
