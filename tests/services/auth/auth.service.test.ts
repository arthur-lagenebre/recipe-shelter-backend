...
describe('AuthService', () => {
    ...
    it('rejects invalid registration inputs and conflicts', async () => {
        await assert.rejects(
            () => service.register({ mail: '', username: 'testuser', password: 'Recipe42?' }),
            (error) => assertHttpError(error, 'AUTH_MISSING_FIELDS', 400)
        );
        await assert.rejects(
            () => service.register({ mail: 'user@example.com', username: 'testuser', password: 'short' }),
            (error) => assertHttpError(error, 'AUTH_WEAK_PASSWORD', 400)
        );

        users.emailTaken = true;
        await assert.rejects(
            () => service.register({ mail: 'user@example.com', username: 'testuser', password: 'Recipe42?' }),
            (error) => assertHttpError(error, 'AUTH_EMAIL_TAKEN', 409)
        );
        users.emailTaken = false;
        users.usernameTaken = true;
        await assert.rejects(
            () => service.register({ mail: 'user@example.com', username: 'testuser', password: 'Recipe42?' }),
            (error) => assertHttpError(error, 'AUTH_USERNAME_TAKEN', 409)
        );
    });
    ...
    it('rejects invalid community credentials and lifecycle states', async () => {
        await assert.rejects(
            () => service.loginCommunity({ mail: '', password: 'Recipe42?' }),
            (error) => assertHttpError(error, 'AUTH_MISSING_FIELDS', 400)
        );
        await assert.rejects(
            () => service.loginCommunity({ mail: 'user@example.com', password: 'Recipe42?' }),
        ...
        users.authUser = { ...baseUser, passwordHash: await bcrypt.hash('Recipe42?', 4) };
        await assert.rejects(
            () => service.loginCommunity({ mail: 'user@example.com', password: 'wrong' }),
            (error) => assertHttpError(error, 'AUTH_INVALID_CREDENTIALS', 401)
        );
        ...
        await assert.rejects(
            () => service.loginCommunity({ mail: 'user@example.com', password: 'Recipe42?' }),
            (error) => assertHttpError(error, 'EMAIL_NOT_VALIDATED', 401)
        );

        users.authUser = { ...baseUser, status: 'banned', passwordHash: await bcrypt.hash('Recipe42?', 4) };
        await assert.rejects(
            () => service.loginCommunity({ mail: 'user@example.com', password: 'Recipe42?' }),
            (error) => assertHttpError(error, 'USER_BANNED', 401)
        );
    });
