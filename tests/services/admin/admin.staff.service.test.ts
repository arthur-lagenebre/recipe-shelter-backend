...
class TestAdminStaffRepository implements AdminStaffRepository {
    ...
    async disable(staffUserId: number, actorStaffUserId: number, reason: string): Promise<number | null> {
        const account = this.accounts.get(staffUserId);
        if (this.disableConflict || !account || account.status !== 'active') return null;
    ...
    async enable(staffUserId: number): Promise<boolean> {
        const account = this.accounts.get(staffUserId);
        if (this.enableConflict || !account || account.status !== 'disabled') return false;
    ...
    async grantRole(staffUserId: number, roleId: number): Promise<boolean> {
        ...
        if (this.grantConflict || !account || !role || account.roles.some((candidate) => candidate.id === roleId)) return false;
    ...
    async revokeRole(staffUserId: number, roleId: number): Promise<boolean> {
        const account = this.accounts.get(staffUserId);
        if (this.revokeConflict || !account) return false;
...
describe('AdminStaffService', () => {
    ...
    it('rejects disabling or revoking the role of another last active SuperAdmin with the same conflict', async () => {
        ...
        await assert.rejects(
            () => service.disable(target.id, actor.id, 'Attempt to disable the final active administrator.', testAdminAuditContext),
        ...
        assert.deepEqual(
            repository.accounts.get(target.id)!.roles.map((role) => role.code),
            ['SuperAdmin']
        );
    ...
    it('returns explicit self-action errors even for the last active SuperAdmin', async () => {
        await assert.rejects(
            () => service.disable(actor.id, actor.id, 'Self-attempt to disable the final active administrator.', testAdminAuditContext),
        ...
        assert.deepEqual(
            repository.accounts.get(actor.id)!.roles.map((role) => role.code),
            ['SuperAdmin']
        );
        assert.equal(audit.inputs.length, 0);
    });
