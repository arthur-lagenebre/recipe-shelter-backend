...
describe('AdminStaffService', () => {
    ...
    it('disables an active account, revokes every active session, then enables it with mandatory audits', async () => {
        ...
        const disabled = await service.disable(target.id, actor.id, 'Confirmed departure from the staff team.', testAdminAuditContext);
        ...
        const enabled = await service.enable(target.id, actor.id, 'Return to the staff team approved.', testAdminAuditContext);
    ...
    it('grants and revokes roles with the exact changed role and reason in the audit', async () => {
        const granted = await service.grantRole(target.id, 'RecipeModerator', actor.id, 'Temporary recipe moderation coverage.', testAdminAuditContext);
        assert.deepEqual(granted.roles.map((role) => role.code), ['RecipeModerator', 'UserAdmin']);

        const revoked = await service.revokeRole(target.id, 'RecipeModerator', actor.id, 'Temporary moderation coverage ended.', testAdminAuditContext);
    ...
    it('rejects disabling or revoking the role of another last active SuperAdmin with the same conflict', async () => {
        ...
        await assert.rejects(() => service.disable(target.id, actor.id, 'Attempt to disable the final active administrator.', testAdminAuditContext), (error) => assertHttpError(error, 409, 'LAST_ACTIVE_SUPER_ADMIN'));
        await assert.rejects(() => service.revokeRole(target.id, 'SuperAdmin', actor.id, 'Attempt to revoke the final administration role.', testAdminAuditContext), (error) => assertHttpError(error, 409, 'LAST_ACTIVE_SUPER_ADMIN'));
    ...
    it('returns explicit self-action errors even for the last active SuperAdmin', async () => {
        await assert.rejects(() => service.disable(actor.id, actor.id, 'Self-attempt to disable the final active administrator.', testAdminAuditContext), (error) => assertHttpError(error, 403, 'STAFF_DISABLE_SELF_FORBIDDEN'));
        await assert.rejects(() => service.revokeRole(actor.id, 'SuperAdmin', actor.id, 'Self-attempt to revoke the final administration role.', testAdminAuditContext), (error) => assertHttpError(error, 403, 'STAFF_ROLE_REVOKE_SELF_FORBIDDEN'));
    ...
    it('allows a SuperAdmin to lose access when another active SuperAdmin remains', async () => {
        ...
        const revoked = await service.revokeRole(target.id, 'SuperAdmin', actor.id, 'A second active SuperAdmin remains available.', testAdminAuditContext);
    ...
    it('rejects every self role mutation to prevent privilege escalation without producing an audit', async () => {
        ...
        await assert.rejects(() => service.disable(actor.id, actor.id, 'Self disable attempt is forbidden.', testAdminAuditContext), (error) => assertHttpError(error, 403, 'STAFF_DISABLE_SELF_FORBIDDEN'));
        await assert.rejects(() => service.grantRole(actor.id, 'RecipeModerator', actor.id, 'Self role grant is forbidden.', testAdminAuditContext), (error) => assertHttpError(error, 403, 'STAFF_ROLE_GRANT_SELF_FORBIDDEN'));
        await assert.rejects(() => service.revokeRole(actor.id, 'SuperAdmin', actor.id, 'Self role removal is forbidden.', testAdminAuditContext), (error) => assertHttpError(error, 403, 'STAFF_ROLE_REVOKE_SELF_FORBIDDEN'));
    ...
    it('rejects unsafe lifecycle and role transitions without producing an audit', async () => {
        ...
        await assert.rejects(() => service.disable(target.id, actor.id, 'short', testAdminAuditContext), (error) => assertHttpError(error, 400, 'STAFF_DISABLE_REASON_TOO_SHORT'));
        await assert.rejects(() => service.enable(target.id, actor.id, 'Account was never disabled.', testAdminAuditContext), (error) => assertHttpError(error, 409, 'STAFF_ENABLE_INVALID_STATUS'));
        await assert.rejects(() => service.grantRole(target.id, 'UserAdmin', actor.id, 'Duplicate role assignment rejected.', testAdminAuditContext), (error) => assertHttpError(error, 409, 'STAFF_ROLE_ALREADY_GRANTED'));
        await assert.rejects(() => service.grantRole(target.id, 'UnknownRole', actor.id, 'Unknown role assignment rejected.', testAdminAuditContext), (error) => assertHttpError(error, 404, 'STAFF_ROLE_NOT_FOUND'));
        await assert.rejects(() => service.get(999, actor.id, testAdminAuditContext), (error) => assertHttpError(error, 404, 'STAFF_USER_NOT_FOUND'));
        assert.equal(audit.inputs.length, 0);
    });
    ...
    it('rejects invalid and concurrent disablement transitions without auditing them', async () => {
        ...
        account.disabledAt = new Date('2026-07-17T12:00:00.000Z');
        await assert.rejects(() => service.disable(target.id, actor.id, 'Repeated disablement request.', testAdminAuditContext), (error) => assertHttpError(error, 409, 'STAFF_ALREADY_DISABLED'));

        account.status = 'invited';
        account.disabledByStaffUserId = null;
        account.disabledReason = null;
        account.disabledAt = null;
        await assert.rejects(() => service.disable(target.id, actor.id, 'Invitation cannot be disabled.', testAdminAuditContext), (error) => assertHttpError(error, 409, 'STAFF_DISABLE_INVALID_STATUS'));

        account.status = 'active';
        repository.disableConflict = true;
        await assert.rejects(() => service.disable(target.id, actor.id, 'Concurrent disablement attempt.', testAdminAuditContext), (error) => assertHttpError(error, 409, 'STAFF_STATUS_CONFLICT'));
    ...
    it('requires MFA and rejects concurrent enablement without auditing it', async () => {
        ...
        await assert.rejects(() => service.enable(target.id, actor.id, 'Enablement without enrolled MFA.', testAdminAuditContext), (error) => assertHttpError(error, 409, 'STAFF_ENABLE_MFA_REQUIRED'));

        account.mfaEnrolledAt = new Date('2026-07-16T10:00:00.000Z');
        repository.enableConflict = true;
        await assert.rejects(() => service.enable(target.id, actor.id, 'Concurrent enablement attempt.', testAdminAuditContext), (error) => assertHttpError(error, 409, 'STAFF_STATUS_CONFLICT'));

        assert.equal(audit.inputs.length, 0);
    });

    it('rejects missing roles, concurrent role changes and invalid reasons without auditing them', async () => {
        await assert.rejects(() => service.revokeRole(target.id, 'RecipeModerator', actor.id, 'Role was never assigned.', testAdminAuditContext), (error) => assertHttpError(error, 409, 'STAFF_ROLE_NOT_GRANTED'));

        repository.grantConflict = true;
        await assert.rejects(() => service.grantRole(target.id, 'RecipeModerator', actor.id, 'Concurrent role grant attempt.', testAdminAuditContext), (error) => assertHttpError(error, 409, 'STAFF_ROLE_CONFLICT'));

        repository.revokeConflict = true;
        await assert.rejects(() => service.revokeRole(target.id, 'UserAdmin', actor.id, 'Concurrent role revoke attempt.', testAdminAuditContext), (error) => assertHttpError(error, 409, 'STAFF_ROLE_CONFLICT'));

        await assert.rejects(() => service.disable(target.id, actor.id, undefined as unknown as string, testAdminAuditContext), (error) => assertHttpError(error, 400, 'STAFF_DISABLE_MISSING_REASON'));
        await assert.rejects(() => service.enable(target.id, actor.id, 'x'.repeat(1001), testAdminAuditContext), (error) => assertHttpError(error, 400, 'STAFF_ENABLE_REASON_TOO_LONG'));
