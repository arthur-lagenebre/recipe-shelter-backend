...
class FakeAdminCatalogProposalRepository implements AdminCatalogProposalRepository {
  ...
  async find(filters: CatalogProposalListFilters, page: PaginationOptions) {
    ...
    const matching = [...this.proposals.values()].filter((proposal) =>
      (filters.status === undefined || proposal.status === filters.status)
      && (filters.proposalType === undefined || proposal.proposalType === filters.proposalType)
      && (filters.recipeId === undefined || proposal.recipeId === filters.recipeId)
      && (filters.authorUserId === undefined || proposal.authorUserId === filters.authorUserId)
...
class FakeCatalogTargets {
  ...
  readonly tagRepository = {
    ...
    findByIdsForUpdate: async (ids: number[]) => ids.flatMap((id) => {
...
describe('AdminCatalogProposalService', () => {
  ...
  beforeEach(() => {
    ...
    service = new AdminCatalogProposalService(
      proposals,
      targets.tagRepository,
      targets.ingredientRepository,
      audit
    );
  });

  it('lists the filtered queue and audits the read', async () => {
    const result = await service.list({ status: 'pending', proposalType: 'ingredient', q: 'lune' }, pagination, actorUserId, testAdminAuditContext);
  ...
  it('accepts an ingredient with a generated slug and a single proposal audit', async () => {
    const reviewed = await service.acceptIngredient(2, {
  ...
  it('rejects either proposal type only with a valid reason and audit history', async () => {
    ...
    await assert.rejects(
      () => service.reject(4, 'court', actorUserId, testAdminAuditContext),
      (error) => assertHttpError(error, 400, 'ADMIN_CATALOG_PROPOSALS_REJECT_REASON_TOO_SHORT')
    );
    assert.equal(audit.inputs.length, 1);
  });

  it('associates proposals only to active entities of the matching catalogue', async () => {
    const tagReview = await service.associateTag(1, {
      targetTagId: 10,
      reason: 'Correspond au tag déjà présent.'
    }, actorUserId, testAdminAuditContext);
    const ingredientReview = await service.associateIngredient(2, {
  ...
  it('converts an ingredient proposal into an alias of an active target', async () => {
    const reviewed = await service.convertIngredientToAlias(4, {
      targetIngredientId: 20,
      languageCode: 'FR',
    ...
    assert.deepEqual(targets.aliases.map((alias) => ({
  ...
  it('rejects missing, reviewed, duplicate-name and concurrent decisions without audit', async () => {
    ...
    await assert.rejects(
      () => service.reject(999, 'Proposition introuvable.', actorUserId, testAdminAuditContext),
      (error) => assertHttpError(error, 404, 'ADMIN_CATALOG_PROPOSALS_NOT_FOUND')
    );
    await assert.rejects(
      () => service.reject(1, 'Décision déjà enregistrée.', actorUserId, testAdminAuditContext),
      (error) => assertHttpError(error, 409, 'ADMIN_CATALOG_PROPOSALS_ALREADY_REVIEWED')
    );
    await assert.rejects(
    ...
    await assert.rejects(
      () => service.reject(3, 'Conflit concurrent de statut.', actorUserId, testAdminAuditContext),
      (error) => assertHttpError(error, 409, 'ADMIN_CATALOG_PROPOSALS_STATUS_CONFLICT')
    );
  ...
  it('fails closed when alias creation conflicts or the audit recorder fails', async () => {
    ...
    await assert.rejects(
      () => service.convertIngredientToAlias(4, {
        targetIngredientId: 20,
        languageCode: 'fr',
    ...
    await assert.rejects(
      () => service.reject(3, 'Refus devant être audité.', actorUserId, testAdminAuditContext),
      /audit unavailable/
    );
  });
  ...
  it('defensively rejects malformed commands, missing groups and inactive or absent targets', async () => {
    const invalidCommands: Array<{ run: () => Promise<unknown>; status: number; code: string }> = [
      ...
      {
        run: () => service.acceptTag(1, { groupId: 8, slug: 'Bad Slug', reason: 'Motif suffisamment long.' }, actorUserId, testAdminAuditContext),
        status: 400,
      ...
      {
        run: () => service.associateIngredient(2, { targetIngredientId: 999, reason: 'Cible ingrédient introuvable.' }, actorUserId, testAdminAuditContext),
        status: 404,
        code: 'ADMIN_CATALOG_PROPOSALS_TARGET_INGREDIENT_NOT_FOUND'
      },
      {
        run: () => service.convertIngredientToAlias(4, { targetIngredientId: 21, languageCode: 'fr', reason: 'Cible ingrédient non active.' }, actorUserId, testAdminAuditContext),
        status: 409,
        code: 'ADMIN_CATALOG_PROPOSALS_TARGET_INGREDIENT_NOT_ACTIVE'
      },
      {
        run: () => service.convertIngredientToAlias(4, { targetIngredientId: 20, languageCode: 'fr_FR', reason: 'Code de langue non valide.' }, actorUserId, testAdminAuditContext),
        status: 400,
